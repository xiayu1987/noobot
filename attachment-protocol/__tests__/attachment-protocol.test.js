...
test("canonical identity is exactly the stable triple", () => {
  ...
  assert.throws(() => parseAttachmentIdentity({ ...identity, path: "/tmp/report.pdf" }), /unknown_attachment_identity_field:path/);
  assert.throws(() => parseAttachmentIdentity({ attachmentId: "att-1", sessionId: "session-1" }), /invalid_attachment_source/);
});

test("metadata identity projection has one strict three-field source", () => {
  assert.deepEqual(projectAttachmentIdentity({
    ...identity,
    id: "legacy-id",
    name: "report.pdf",
    path: "/tmp/report.pdf",
...
test("descriptor is metadata and cannot smuggle path access fields", () => {
  ...
  assert.throws(() => parseAttachmentDescriptor({ ...descriptor(), path: "/tmp/report.pdf" }), /unknown_attachment_descriptor_field:path/);
});
...
test("persistence, runtime, access and UI models stay separate", () => {
  ...
  assert.deepEqual(parseRuntimeAttachmentRef({ identity, turnScopeId: "turn-1", runId: "run-1" }).identity, identity);
  assert.equal(parseAttachmentAccessRef({ identity, capability: "download", href: "/attachments/att-1" }).capability, "download");
  assert.equal(parseAttachmentUiView({ identity, name: "report.pdf", mimeType: "application/pdf" }).name, "report.pdf");
  assert.throws(() => parsePersistedAttachmentRecord({
    identity,
...
test("lifecycle events require stable identity and versioned message identity", () => {
  ...
  assert.throws(() => createAttachmentLifecycleEvent({ ...event, eventVersion: 2 }), /unsupported_attachment_event_version/);
  assert.throws(() => createAttachmentLifecycleEvent({ ...event, identity: { ...identity, sessionId: "" } }), /invalid_attachment_session_id/);
});
...
test("attachment set update distinguishes unchanged from explicit delete-all", () => {
  ...
  assert.throws(() => createAttachmentSetUpdate([identity, identity]), /duplicate_attachment_identity/);
