  ...
  execute_native_script: {
    ...
    params: {
      script_body: {
        ...
        text: "Async function body. Available bindings: browser, libreoffice, ffmpeg, ffprobe, files, output, args, and log(message). Exact signatures: await ffmpeg.run({ args: [...] }); await ffprobe.run({ args: [...] }); await libreoffice.convert({ input, outputDirectory, outputFormat }). First obtain input:// tokens with await files.input(index), and create output or temporary tokens with await output.file(...) or await output.tempFile(...). LibreOffice input may be an existing input://, output://, or temp:// file from this task. browser.newPage() returns a restricted page supporting goto, setContent, title, url, content, DOM operations, screenshot, and close; evaluate is unavailable. files.readText/readJson read all three token kinds; files.writeText/writeJson write only output:// tokens. Script return values are not file outputs; formal artifacts must be written to output://.",
      },
      inputs: {
