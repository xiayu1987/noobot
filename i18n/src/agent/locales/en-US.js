...
export default {
  ...
  "tools.patch_file.fieldPatch": "Patch content; read the file first and use exact context. Unified diff may use standard @@ -line +line @@ headers and also tolerates search-style @@ hunks. Patch paths resolve from the current workspace root; pass root or include the child project directory when target files are under a child project.",
  "tools.patch_file.fieldStrip": "Diff path strip hint; common forms such as a/, b/, no prefix, and /project are tried automatically when unsure.",
  "tools.patch_file.fieldRoot": "Patch root directory (optional, relative to workspace); when target files are under one child project, pass that child project path as root.",
