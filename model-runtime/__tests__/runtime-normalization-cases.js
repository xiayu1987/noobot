...
test("cache fields are selected explicitly and their values are generated by the protocol", () => {
  const common = { adapterId: "openai-compatible", modelFamily: "generic" };
  ...
  assert.equal("prompt_cache_key" in openAi, false);
  assert.equal("prompt_cache_options" in openAi, false);
  assert.equal("prompt_cache_retention" in openAi, false);
  assert.equal("cached_content" in openAi, false);
  assert.equal("cache_control" in openAi, false);

  const declared = compileProviderModelKwargs(
    {
      ...common,
      operatorId: "generic",
      model: "vendor-routed-model",
      prompt_cache_fields: ["prompt_cache_retention", "prompt_cache_key", "prompt_cache_options"],
    },
    "workflow.plan",
  );
  assert.deepEqual(declared, {
    prompt_cache_key: "noobot-workflow-plan-vendor-routed-model",
  ...
  assert.deepEqual(
    compileProviderModelKwargs({
      ...
      operatorId: "generic",
      model: "vendor-routed-model",
      prompt_cache_fields: ["prompt_cache_options"],
    }),
    { prompt_cache_options: { ttl: "30m" } },
  );
  assert.throws(
    ...
    /prompt_cache_fields must be an array/,
  );
  assert.throws(
    () =>
      compileProviderModelKwargs({
        ...
        operatorId: "generic",
        model: "vendor-routed-model",
        prompt_cache_fields: ["unknown_cache_field"],
      }),
    /unsupported model spec\.prompt_cache_fields value: unknown_cache_field/,
  );
  assert.throws(
    () =>
      compileProviderModelKwargs({
        ...
        operatorId: "generic",
        model: "vendor-routed-model",
        prompt_cache_fields: ["prompt_cache_key", "prompt_cache_key"],
      }),
    /duplicate model spec\.prompt_cache_fields value: prompt_cache_key/,
  );
  assert.deepEqual(
    compileProviderModelKwargs({
      ...common,
  ...
  assert.throws(
    ...
    /prompt_cache_options is outside the model-family cache protocol for claude-opus-5/,
  );
  ...
  assert.deepEqual(
    compileProviderModelKwargs({
      ...
      extra_body: { cache_control: { type: "forged", ttl: "1h" } },
    }),
  ...
  const kimi = compileProviderModelKwargs({
    ...common,
    operatorId: "kimi",
    model: "kimi-k3",
    modelFamily: "kimi",
  ...
  assert.deepEqual(kimi, {
    prompt_cache_key: "noobot-main-kimi-k3",
