...
test("buildContextMessageBlocks replays 71ad4373 stopped snapshot without tool or user_meta degradation", () => {
  ...
  assert.equal(
    blocks.history.filter((message) => String(message?.content || "").includes("[用户元信息]"))
      .length,
    1,
  );
  assert.equal(
  ...
  assert.deepEqual(
    aiMessages.map((message) => message.tool_calls?.[0]?.id),
    toolCallIds,
  );
  assert.deepEqual(
  ...
  const userMetaMessages = blocks.incremental.filter((message) =>
    String(message?.content || "").includes("[用户元信息]"),
  );
...
test("buildContextMessageBlocks resume keeps tool pairing, single user_meta and no guidance meta", () => {
  ...
  const history = [
    ...
    tool({ tool_call_id: toolCallId, content: '{"ok":true}' }),
    human({
      content: "继续推进任务",
      additional_kwargs: {
  ...
  assert.equal(
    metaPayloads.some((payload) => payload.dialogProcessId === guidanceDialogProcessId),
    false,
  );
...
test("buildContextMessageBlocks rebuilds user_meta on first continue from 4c18984a stopped snapshot", () => {
  ...
  const history = [
    ...
    tool({ tool_call_id: toolCallId, content: "write_file result" }),
  ];
  ...
  const historicalMeta = JSON.parse(
    String(blocks.history[1]?.content || "{}").match(/\{[\s\S]*\}/)?.[0] || "{}",
  );
  ...
  const currentMeta = JSON.parse(
    String(blocks.incremental[1]?.content || "{}").match(/\{[\s\S]*\}/)?.[0] || "{}",
  );
...
test("buildContextMessageBlocks keeps resumed incremental user identity and scoped attachments from 1ec8e12c resend chain", () => {
  ...
  assert.equal(
    humanMessages.some((message) => message.content === "继续前同一快照里的上一条用户消息"),
    true,
  );
  ...
  const metaPayloads = humanMessages
    .filter((message) => message.additional_kwargs?.noobotInternalMessageType === "user_meta")
    .map((message) =>
      JSON.parse(String(message.content || "{}").match(/\{[\s\S]*\}/)?.[0] || "{}"),
    );
  const firstResendMeta = metaPayloads.find(
    (payload) => payload.dialogProcessId === firstResendDialogProcessId,
  );
  const firstStopMeta = metaPayloads.find(
    (payload) => payload.dialogProcessId === "dlg-1ec8e12c-first-stop",
  );
  const currentMeta = metaPayloads.find(
    (payload) => payload.dialogProcessId === currentDialogProcessId,
  );
...
test("buildContextMessageBlocks rebuilds user_meta on first stopped snapshot resume", () => {
  ...
  const blocks = buildContextMessageBlocks(
    createTestAgentExecutionScope(
      ...
      { messageBlocks: { system: ["snapshot system"], history } },
    ),
    {
      currentUserMessage: createPersistedCurrentUserMessage("继续", {
        userName: "admin",
  ...
  const historyMetaPayload = JSON.parse(
    blocks.history[1].content.match(/\n([\s\S]*)\n\[\//)?.[1] || "{}",
  );
  ...
  const incrementalMetaPayload = JSON.parse(
    blocks.incremental[1].content.match(/\n([\s\S]*)\n\[\//)?.[1] || "{}",
  );
