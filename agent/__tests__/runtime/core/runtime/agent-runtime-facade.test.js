...
import { AgentRuntimeFacade } from "../../../../src/runtime/facade/agent-runtime-facade.js";
...
test("AgentRuntimeFacade prepareTurnExecution builds context and runtime context", async () => {
  ...
  const result = await facade.prepareTurnExecution({
    buildContextPayload: { sessionId: "s1", userMessageAttachments: [{ attachmentId: "att1" }] },
    abortSignal,
  });
  ...
  assert.deepEqual(calls[0]?.[1]?.userMessageAttachments, [{ attachmentId: "att1" }]);
