...
const event = (eventType, eventId, sequence, extra = {}) => ({
  ...
  eventType, eventId, sequence, messageId: "model-message-1",
  presentationMessageId: "presentation-1", sequenceScopeId: "model-message-1",
  sequenceDomain: "message-event", authority: "authoritative",
  timestamp: `2026-07-29T01:00:0${sequence}.000Z`, toolCallId: "call-1", tool: "read_file",
  ...extra,
});

test("canonical tool reducer merges call and result by stable toolCallId", () => {
  let timeline = reduceCanonicalToolTimeline([], event("tool_call_start", "event-1", 1, { args: { path: "README.md" } }));
  timeline = reduceCanonicalToolTimeline(timeline, event("tool_call_end", "event-2", 2, { result: { ok: true } }));
