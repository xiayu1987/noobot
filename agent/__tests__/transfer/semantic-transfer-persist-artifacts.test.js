...
test("persistTransferArtifacts saves through attachment service and returns transfer envelopes", async () => {
  const { getTransferAttachmentMetas } = await import("../../src/transfer/index.js");
  const { persistTransferArtifacts } = await import("../../src/transfer/storage/attachment-adapter.js");
  const calls = [];
...
test("attachment metadata normalizes owner and turn scope shapes", async () => {
  const { mapAttachmentRecordsToMetas } = await import("../../src/artifacts/index.js");
