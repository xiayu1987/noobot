  ...
  const reader = service([
      ...
      turnLifecycle: {
        turns: {
          child: turn({
            ...
            rootExecutionId: "agent:root",
            origin: { type: "workflow_node", workflowNodeExecutionId: "node-1" },
          }),
        },
      },
    },
    {
      ...
      turnLifecycle: {
        turns: {
          grandchild: turn({
            turnScopeId: "turn-grandchild",
            executionId: "agent:grandchild",
            parentExecutionId: "agent:child",
            rootExecutionId: "agent:root",
          }),
        },
      },
...
test("execution recovery keeps lifecycle and thinking timing as separate authoritative facts", async () => {
  const reader = service([
    {
  ...
  assert.equal(result.execution.startedAt, "");
  assert.equal(result.execution.finishedAt, "");
  assert.equal(result.execution.thinkingStartedAt, "2026-07-20T00:00:01.000Z");
  assert.equal(result.execution.thinkingFinishedAt, "");
});
...
test("children query returns direct children only and preserves historical attempts", async () => {
  const reader = service([
    ...
    {
      sessionId: "nested",
      parentSessionId: "attempt-2",
      turnLifecycle: {
        turns: {
          nested: turn({
            turnScopeId: "nested",
            executionId: "agent:nested",
            parentExecutionId: "agent:attempt-2",
          }),
        },
      },
    },
  ...
  assert.deepEqual(result.children.map((item) => item.executionId).sort(), [
    "agent:attempt-1",
    "agent:attempt-2",
  ]);
});
...
test("duplicate executionId with different ownership is diagnosed and excluded", async () => {
  const reader = service([
    {
      sessionId: "left",
      turnLifecycle: { turns: { left: turn({ executionId: "agent:duplicate" }) } },
    },
    {
      sessionId: "right",
      turnLifecycle: { turns: { right: turn({ executionId: "agent:duplicate" }) } },
    },
  ]);
  const result = await reader.getExecution({ userId: "u1", executionId: "agent:duplicate" });
  ...
  assert.deepEqual(
    result.conflict.identities.map((item) => item.sessionId),
    ["left", "right"],
  );
});
  ...
  const reader = service([
    {
      ...
      turnLifecycle: {
        turns: {
          orphan: turn({
            executionId: "agent:orphan",
            parentExecutionId: "agent:missing",
            rootExecutionId: "agent:missing",
          }),
        },
      },
    },
...
test("disposable read index is isolated by user and rebuilds when authoritative summaries change", async () => {
  ...
  const sessionsByUser = {
    u1: [
      {
        sessionId: "u1-session",
        updatedAt: "2026-07-20T00:00:00.000Z",
        turnLifecycle: { turns: { root: turn() } },
      },
    ],
    u2: [
      {
        ...
        updatedAt: "2026-07-20T00:00:00.000Z",
        turnLifecycle: {
          turns: { root: turn({ executionId: "agent:u2", rootExecutionId: "agent:u2" }) },
        },
      },
    ],
  };
  ...
  assert.equal(
    (await reader.getExecution({ userId: "u1", executionId: "agent:root" })).found,
    true,
  );
  assert.equal(
    (await reader.getExecution({ userId: "u1", executionId: "agent:root" })).found,
  ...
  sessionsByUser.u1[0].turnLifecycle.turns.root = turn({
    revision: 4,
    sequence: 4,
...
test("summary/index failure falls back to authoritative scan without caching stale facts", async () => {
    ...
    sessionCrudService: {
      ...
      async getAllSessionsData() {
        ...
        return [
          {
            sessionId: "root",
            turnLifecycle: { turns: { root: turn({ revision, sequence: revision }) } },
          },
        ];
      },
    },
  ...
  assert.equal(
    (await reader.getExecution({ userId: "u1", executionId: "agent:root" })).execution.revision,
    2,
  );
  revision = 3;
  assert.equal(
