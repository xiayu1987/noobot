...
function validateScriptBody(value) {
  ...
  if (Buffer.byteLength(body, "utf8") > LENGTH_THRESHOLDS.nativeScript.sourceBytes) throw new Error("script_body exceeds 100 KB");
  let ast;
  try {
    ast = parse(`async ({ browser, libreoffice, ffmpeg, ffprobe, files, output, args, log }) => {\n${body}\n}`, { ecmaVersion: "latest", sourceType: "module" });
  } catch (error) {
    throw new Error(`script_body syntax error: ${error.message}`);
  }
  const visit = (node) => {
    if (!node || typeof node !== "object") return;
    if (["ImportDeclaration", "ImportExpression", "ExportNamedDeclaration", "ExportDefaultDeclaration", "ExportAllDeclaration", "MetaProperty", "ThisExpression"].includes(node.type)) {
    ...
    if (node.type === "MemberExpression") {
      const propertyName = node.computed && node.property?.type === "Literal"
        ? String(node.property.value || "")
        : !node.computed && node.property?.type === "Identifier" ? node.property.name : "";
      if (FORBIDDEN_PROPERTIES.has(propertyName)) throw new Error(`script_body contains forbidden property access: ${propertyName}`);
...
function runGeneratedScript({ scriptPath, cwd, env, timeoutMs, abortSignal }) {
  return new Promise((resolve) => {
    ...
    const stdout = []; const stderr = []; let bytes = 0; let timedOut = false; let aborted = false;
    const capture = (target, chunk) => { if (bytes < LENGTH_THRESHOLDS.nativeScript.processOutputBytes) { target.push(Buffer.from(chunk).subarray(0, LENGTH_THRESHOLDS.nativeScript.processOutputBytes - bytes)); bytes += chunk.length; } };
    ...
    const timer = setTimeout(() => terminate("timeout"), timeoutMs); timer.unref?.();
    child.on("close", (code, signal) => { clearTimeout(timer); if (forceKillTimer) clearTimeout(forceKillTimer); abortSignal?.removeEventListener?.("abort", onAbort); resolve({ code: timedOut ? 124 : aborted ? 130 : Number(code || 0), signal: signal || "", stdout: Buffer.concat(stdout).toString("utf8"), stderr: Buffer.concat(stderr).toString("utf8") }); });
  });
}
...
async function collectOutputFiles(root, relative = "") {
  ...
  for (const entry of entries) {
    if (entry.isSymbolicLink()) throw new Error("native script output cannot contain symbolic links");
...
export function createNativeScriptTool({ agentContext }) {
  ...
  const tool = new DynamicStructuredTool({
    ...
    schema: z.object({
      ...
      inputs: z.array(createFileInputSchema({
        filePathDescription: tTool(runtime, "tools.nativeScript.fieldFilePath"),
        attachmentIdentityDescription: tTool(runtime, "tools.nativeScript.fieldAttachmentIdentity"),
      })).optional().default([]).describe(tTool(runtime, "tools.nativeScript.fieldInputs")),
      arguments: z.record(z.string(), z.unknown()).optional().default({}).describe(tTool(runtime, "tools.nativeScript.fieldArguments")),
    }),
    func: async ({ script_body, inputs = [], arguments: scriptArguments = {} }, _runManager, toolConfig = {}) => {
      const identity = toolConfig?.configurable?.transferIdentity;
      if (!identity || typeof identity !== "object") throw new Error("native_script_identity_required");
      const body = validateScriptBody(script_body);
      await confirmCriticalToolOperation({ runtime, riskLevel: TOOL_RISK_LEVEL.HIGH, toolName: TOOL_NAME.EXECUTE_NATIVE_SCRIPT, operation: "execute native capability script", reason: "The script can invoke browser, LibreOffice, and FFmpeg capabilities." });
      ...
      try {
        ...
        for (const [index, value] of (Array.isArray(inputs) ? inputs : []).entries()) {
          const resolvedInput = await resolveFileInput({ ...value, agentContext, fieldName: "inputs", capability: PATH_CAPABILITIES.NATIVE_INPUT });
          ...
          if (totalInputBytes > LENGTH_THRESHOLDS.nativeScript.inputTotalBytes) throw new Error("native script inputs exceed 200 MB");
        ...
        return toToolJsonResult(TOOL_NAME.EXECUTE_NATIVE_SCRIPT, {
          ...
          stderr: projectNativeOutput(error?.message || String(error || "native script failed"), { inputRoot, outputRoot, tempRoot }),
          output_file_count: 0,
          output_bytes: 0,
          transferEnvelopes: [],
        });
