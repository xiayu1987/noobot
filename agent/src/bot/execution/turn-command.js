...
export function createTurnCommand({
  ...
  const origin =
    trim(caller).toLowerCase() === "bot" ? TURN_COMMAND_ORIGIN.INTERNAL : TURN_COMMAND_ORIGIN.USER;
  const command = {
    type: resume ? TURN_COMMAND_TYPE.CONTINUE : TURN_COMMAND_TYPE.SEND,
    origin,
    userId: trim(userId),
    sessionId: trim(sessionId),
    parentSessionId: trim(parentSessionId),
    dialogProcessId: trim(dialogProcessId),
    parentDialogProcessId: trim(parentDialogProcessId),
    turnScopeId: trim(turnScopeId),
...
export function toCommitTurnPayload(command = {}) {
  return {
    ...
    dialogProcessId: command.dialogProcessId,
    parentDialogProcessId: command.parentDialogProcessId,
    turnScopeId: command.turnScopeId,
    action: command.type,
    messageOrigin: command.origin === TURN_COMMAND_ORIGIN.USER ? "natural" : "internal",
    userMetaMaterialized: command.origin === TURN_COMMAND_ORIGIN.USER,
