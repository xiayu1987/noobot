...
import { validateSessionProvisionIntent } from "@noobot/session-protocol";
...
export async function recordAuthorityEventAttempt({ userId, sessionId, parentSessionId = "", persistenceContext = null, eventId = "" } = {}) {
  ...
  return this._withSessionMutation(userId, sessionId, async () => {
    ...
    return { recorded: true, event: result.outbox.find((item) => item.eventId === eventId), aggregateVersion: resolveAggregateVersion(session) };
...
export async function acknowledgeAuthorityEvent({ userId, sessionId, parentSessionId = "", persistenceContext = null, eventId = "" } = {}) {
  ...
  return this._withSessionMutation(userId, sessionId, async () => {
    ...
    if (!result.changed) return { acknowledged: true, deduplicated: true, aggregateVersion: actualVersion };
    session.authorityEventOutbox = result.outbox;
    session.updatedAt = this.now();
    await this.sessionRepo.save(userId, session, resolvedParentSessionId, { expectedAggregateVersion: actualVersion, persistenceContext });
...
export async function applyTurnLifecycleEvent({
  ...
  if (provisionIntent.requested) {
    return provisionSessionWithInitialTurn.call(this, {
      userId, sessionId, parentSessionId, persistenceContext, expectedAggregateVersion, ...event,
    });
  }
  return this._withSessionMutation(userId, sessionId, async () => {
    ...
    const actualVersion = resolveAggregateVersion(session);
    if (expectedAggregateVersion !== undefined && Number(expectedAggregateVersion) !== actualVersion) {
...
export async function provisionSessionWithInitialTurn({
  userId, sessionId, parentSessionId = "", persistenceContext = null, expectedAggregateVersion, createSessionIfAbsent, ...event
} = {}) {
  ...
  return this._withSessionMutation(userId, sessionId, async () => {
    ...
    if (!result.applied) return { ...result, session: isNew ? null : session, aggregateVersion: actualVersion };
    session.turnLifecycle = result.lifecycle;
    session.authorityEventOutbox = result.eventOutbox;
    session.updatedAt = this.now();
    if (session.shortMemoryCheckpoint === undefined) session.shortMemoryCheckpoint = 0;
    ...
    return { ...result, session, aggregateVersion: resolveAggregateVersion(session), sessionCreated: isNew };
  }, parentSessionId, persistenceContext);
}
