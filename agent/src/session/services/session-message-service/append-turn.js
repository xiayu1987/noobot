...
function upsertTurnInSession(
  ...
  const turn = normalizeMessageEntity(
    {
      ...
      ...(String(thinkingStartedAt || "").trim()
        ? { thinkingStartedAt: String(thinkingStartedAt || "").trim() }
        : {}),
      ...(String(thinkingFinishedAt || "").trim()
        ? { thinkingFinishedAt: String(thinkingFinishedAt || "").trim() }
        : {}),
      ts: service.now(),
    },
    service.now,
  ...
  const preferredAttachments =
    Array.isArray(turn?.transferEnvelopes) && turn.transferEnvelopes.length
      ? []
  ...
  const turnDialogProcessId = resolveContextMessageDialogProcessId(turn);
  ...
  const compositeIdentityIndex = turn.messageId
    ? session.messages.findIndex(
        (message = {}) =>
          ...
          resolveContextMessageDialogProcessId(message) === turnDialogProcessId &&
          String(message?.turnScopeId || "").trim() === turnScope,
      )
    : -1;
  const existingIndex = normalizedMessageUid
    ? session.messages.findIndex(
        (message = {}) => String(message?.messageUid || "").trim() === normalizedMessageUid,
      )
  ...
  if (existingIndex >= 0) {
    ...
    if (
      ...
      (resolveContextMessageDialogProcessId(existing) !== turnDialogProcessId ||
