...
7. Persist until verified with the task central: prioritize completing the current task; unless required information is missing, a high-risk action needs authorization, or tools/environment are blocked, continue until completion is verifiable; final replies should only summarize completed work, changed files, verification results, and remaining risks.
