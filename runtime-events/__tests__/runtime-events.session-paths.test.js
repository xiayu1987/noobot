...
test("placeholder parent session id cannot create an unpersisted session directory", async () => {
  ...
  assert.equal(
    await pathExists(path.join(workspaceRoot, "admin", "runtime", "session", "draft-session")),
    false,
  );
});
  ...
  const result = await writeRuntimeEvent(
    {
      ...
      category: "debug",
      level: "debug",
      event: "frontend.workflowRender.draft",
      userId: "admin",
      sessionId: "draft-session",
      workspaceRoot,
      data: { debugType: "workflow-diagnostics" },
    },
