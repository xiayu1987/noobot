...
test("runtime event archive cleanup deletes archives older than retention days", async () => {
  ...
  const seed = await writeRuntimeEvent(
    { ...baseEvent, event: "agent.runtime.cleanup.seed" },
    {
      maxFileBytes: 0,
      retentionDays: 0,
      maxArchives: 0,
  ...
  const result = await writeRuntimeEvent(
    { ...baseEvent, event: "agent.runtime.cleanup.ttl" },
    {
      maxFileBytes: 0,
      retentionDays: 1,
      maxArchives: 0,
...
test("runtime event archive cleanup keeps only newest max archives", async () => {
  ...
  const seed = await writeRuntimeEvent(
    { ...baseEvent, event: "agent.runtime.cleanup.maxSeed" },
    {
      maxFileBytes: 0,
      retentionDays: 0,
      maxArchives: 0,
  ...
  const result = await writeRuntimeEvent(
    { ...baseEvent, event: "agent.runtime.cleanup.max" },
    {
      maxFileBytes: 0,
      retentionDays: 0,
      maxArchives: 2,
...
test("runtime event archive cleanup can be disabled", async () => {
  ...
  const seed = await writeRuntimeEvent(
    { ...baseEvent, event: "agent.runtime.cleanup.disabledSeed" },
    {
      maxFileBytes: 0,
      retentionDays: 0,
      maxArchives: 0,
  ...
  const archive = await writeArchive(
    path.join(path.dirname(seed.file), "system.20000101T000000Z.jsonl"),
    30 * 24 * 60 * 60 * 1000,
  );

  const result = await writeRuntimeEvent(
    { ...baseEvent, event: "agent.runtime.cleanup.disabled" },
    {
      maxFileBytes: 0,
      retentionDays: 0,
      maxArchives: 0,
...
test("runtime event archive cleanup ignores active and unrelated jsonl files", async () => {
  ...
  const seed = await writeRuntimeEvent(
    { ...baseEvent, event: "agent.runtime.cleanup.activeSeed" },
    {
      maxFileBytes: 0,
      retentionDays: 0,
      maxArchives: 0,
  ...
  const unrelated = await writeArchive(
    path.join(path.dirname(seed.file), "interaction.20000101T000000Z.jsonl"),
    30 * 24 * 60 * 60 * 1000,
  );

  const result = await writeRuntimeEvent(
    { ...baseEvent, event: "agent.runtime.cleanup.active" },
    {
      maxFileBytes: 0,
      retentionDays: 1,
      maxArchives: 0,
