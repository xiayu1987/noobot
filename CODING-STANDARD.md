...
## 4. Model Protocol and Runtime / 模型协议与运行时

### EN

- `@noobot/model-protocol` is the single contract for provider configuration and model capabilities.
- `@noobot/model-runtime` is the only provider-identification, model-family policy, request-adaptation, and execution layer.
- `openai_compatible` is the only transport format. Provider and model-family differences are data-driven policies selected from the resolved endpoint/model facts, not alternate wire formats or call-site branches.
- Multimodal and reasoning behavior must be declared by model/provider configuration and validated at the runtime boundary.

### 中文

- `@noobot/model-protocol` 是 provider 配置与模型能力的唯一契约。
- `@noobot/model-runtime` 是唯一的供应商识别、模型系列策略、请求适配和执行层。
- `openai_compatible` 是唯一传输格式。供应商和模型系列差异由解析后的端点/模型事实选择数据化策略，不新增 wire format 或调用点分支。
- 多模态和推理能力必须由模型/provider 配置声明，并在运行时边界校验。
...
### 8.1 No `console.*` in Core Business Code / 核心业务代码禁止使用 `console.*`

#### EN

- Do NOT use `console.log`, `console.error`, `console.warn`, `console.info`, or `console.debug` in any core business module under `agent/src/`.
...
- `agent/src/observability/` — the logging infrastructure module itself

#### 中文

- 禁止在 `agent/src/` 下的任何核心业务模块中使用 `console.log`、`console.error`、`console.warn`、`console.info` 或 `console.debug`。
...
- `agent/src/observability/` — 日志基础设施模块本身

---

### 8.2 Use `errorLogger` for Structured Logging / 使用 `errorLogger` 进行结构化日志记录
...
### 8.5 Optional Logger Boundary / 可选日志边界

#### EN

- Functions that declare an optional `errorLogger` boundary MUST use `= null` and handle its absence explicitly.

#### 中文

- 声明可选 `errorLogger` 边界的函数必须使用 `= null`，并显式处理日志器不存在的情况。
...
- [ ] Model/provider behavior remains inside `model-protocol` and `model-runtime`.
...
- [ ] 业务逻辑中未新增魔法字符串。
- [ ] 未新增硬编码用户文案。
- [ ] 模型/provider 行为仍收敛在 `model-protocol` 与 `model-runtime`。
