...
if (legacyWindowHits.length) {
  fail(
    ...
    legacyWindowHits
      ...
      .map(
        (item) =>
          `  ${rel(item.file)}:${lineOf(readFileSync(item.file, "utf8"), item.index)} ${item.token}`,
      )
      .join("\n"),
  );
} else {
...
if (
  !failures.some((item) => item.title.includes("legacy context/message-block compatibility helper"))
) {
...
const summaryMutationCallPattern =
...
function validateRequestLogOrder(logPath = "") {
  ...
  for (const [bodyIndex, bodyText] of bodies.entries()) {
    ...
    checked += 1;
    const systemLikeCount = messages.filter((message) =>
      isSystemLikeLogRole(normalizeLogRole(message)),
    ).length;
    const systemLikeContents = messages
      .filter((message) => isSystemLikeLogRole(normalizeLogRole(message)))
      .map((message) => String(message?.content || ""));
    const hasOnlyHarnessSystem =
      systemLikeCount > 0 &&
      systemLikeContents.every(
        (content) =>
