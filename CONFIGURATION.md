...
| `security.trusted_directories`                              | string[]     | Trusted host directories used for file-tool risk classification; defaults to `["*"]`                        |
...
`execute_native_script` injects controlled Playwright, LibreOffice, FFmpeg/FFprobe, declared-input, task-output, and user-interaction capabilities. Its unique file protocol is `files.input`, `files.readText`, `files.readJson`, `files.writeText`, `files.writeJson`, `output.file`, `output.tempFile`, and `output.directory`. Reads accept `input://`, `output://`, and `temp://` task paths; writes accept only `output://`. Capability wrappers resolve task paths internally. It does not expose imports, shell commands, environment variables, executable selection, or arbitrary host paths. Capability options are declared on each capability method, so the tool schema stays limited to `script_body`, `inputs`, `arguments`, and `riskLevel`. Browser navigation accepts HTTP(S) only, plus `data:`, `blob:`, and `about:` resources; hosts are not restricted to loopback. Outputs are persisted through semantic-transfer. This host-restricted mode is intended for trusted local/admin automation; it is not an operating-system security sandbox for hostile code.

`browser.newPage({ headed, profile })` resolves against a persistent Chromium profile owned by the agent process rather than a throwaway context. Each profile lives at `<profile root>/<user id>/<profile name>`, where the profile root comes from `NOOBOT_BROWSER_PROFILE_ROOT` (the desktop client points it at `<userData>/browser-profiles`) and otherwise falls back to `<base path>/runtime/browser-profiles`. The owning user id is injected by the agent and can never be supplied by the script; only `profile` is script-controlled and it is restricted to a traversal-safe name. Because a browser window must outlive the script that opened it, the window stays open after the script ends and is released only by `browser.closeProfile({ profile })` or by the user closing it. `headed: true` requires an active display session, and one profile cannot be opened in headed and headless mode at the same time.

A persistent profile is a deliberate widening of script isolation: cookies, `localStorage`, and `IndexedDB` written during a signed-in session survive across scripts, turns, and restarts, so any later script running as the same user can reach every login state accumulated in that profile. Profiles are isolated per user id and per profile name, and never per script or per turn. In headed mode the request filter is also relaxed so interactive sign-in through third-party identity providers and CDNs can complete. Use a dedicated profile name to keep unrelated credentials apart, and prefer the default headless mode when a script does not need a signed-in session. When the profile root falls back outside a desktop client and lands on a temporary directory, login state does not survive a restart.

`ui.waitForUser({ content, fields })` suspends the script until the user answers through the standard interaction channel, which is what makes a manual sign-in inside a headed browser possible. The script timeout clock is stopped while an answer is pending and resumes once every pending request is resolved, so human latency cannot trigger a timeout kill. A cancelled interaction fails the script.
...
`security.trusted_directories` is global-admin configuration and affects risk classification only; it never grants file access. When omitted, its effective value is `["*"]`, so every host directory except the platform-protected roots and `security.path_policy.roles.super_admin.host.denied_roots` is trusted. When concrete absolute paths are configured, only those directories and their descendants are trusted. An empty array trusts no host directory. Relative entries and configurations that combine `"*"` with concrete paths are rejected by the path protocol. Protected or denied roots always remain untrusted, even if `"*"` or a parent directory is configured. Trusted host file reads/searches use workspace-equivalent risk, and trusted writes/patches use `medium` risk. Arbitrary host shell execution remains `critical` because a shell command cannot be confined to the listed directories by path classification alone.
