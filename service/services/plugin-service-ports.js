...
async function readSegmentedExecutionLogs({ workspacePath, rootSessionId, childSessionId, skip = 0, limit = Infinity }) {
  ...
  if (!rootSessionId || !childSessionId || !relative || relative.startsWith("..") || path.isAbsolute(relative)) return [];
  let entries = [];
  try { entries = await fs.readdir(eventsDir, { withFileTypes: true }); } catch { return []; }
  const names = entries.filter((entry) => entry.isFile() && /^segment-\d+\.jsonl$/.test(entry.name)).map((entry) => entry.name).sort();
  const logs = [];
  let seen = 0;
...
export function createPluginServicePorts({ bot = null, translateText = null } = {}) {
  function resolveWorkflowDir({ userId, sessionId, dialogProcessId, locale = "" }) {
    ...
    const outputDir = path.resolve(workspacePath, "runtime/workflow/session", String(sessionId || "").trim(), String(dialogProcessId || "").trim());
  ...
  return Object.freeze({
    ...
    sessions: Object.freeze({
      async readWorkflowSnapshot({ userId, sessionId, dialogProcessId, locale, executionPage = null }) {
        const { workspacePath, outputDir } = resolveWorkflowDir({ userId, sessionId, dialogProcessId, locale });
        let entries = [];
        try { entries = await fs.readdir(outputDir); } catch {}
        const snapshot = await readSessionArtifactSnapshot({
          outputDir,
          executionLogOptions: executionPage ? { skip: executionPage.cursor, limit: executionPage.limit + 1 } : {},
        });
        const childSessionId = String(snapshot.sessionSummary?.sessionId || snapshot.session?.sessionId || "").trim();
        const scopedLogs = Array.isArray(snapshot.executionLogs) ? snapshot.executionLogs : [];
        const hasScopedArtifacts = entries.includes("execution-events") || entries.includes("execution-events.jsonl");
        const executionLogs = scopedLogs.length || (executionPage && hasScopedArtifacts)
          ? scopedLogs
          : await readSegmentedExecutionLogs({ workspacePath, rootSessionId: sessionId, childSessionId, skip: executionPage?.cursor || 0, limit: executionPage ? executionPage.limit + 1 : Infinity });
