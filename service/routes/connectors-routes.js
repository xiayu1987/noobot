...
function mergeRuntimeAndHistoryConnectorGroup({
  ...
  return mergeConnectorStatusItems({
    runtimeConnectors,
    historyConnectors,
    disconnectedMessage: translateText("status.disconnectedFromHistory", locale),
  });
}
...
export function registerConnectorRoutes(
  ...
  app.get(
    ...
    withJsonError(
      async (req, res) => {
        ...
        const allMergedConnectors = [...mergedDatabases, ...mergedTerminals, ...mergedEmails];
  ...
  app.put(
    ...
    withJsonError(
      async (req, res) => {
        const { userId, sessionId } = req.params;
        const selectedConnectors = normalizeSelectedConnectors(req.body?.selectedConnectors);
        const rootSessionId = await bot.session.getRootSessionId({ userId, sessionId });
