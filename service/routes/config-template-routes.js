...
export function registerConfigAndTemplateRoutes(
  ...
  function authorizeConfigParamScope(req, res, scope) {
    if (scope !== "system" || isSuperAdminRole(req?.auth?.role)) return true;
  ...
  app.get(
    ...
    withJsonError(
      async (req, res) => {
        ...
        if (!authorizeConfigParamScope(req, res, scope)) return;
