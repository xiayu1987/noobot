 ...
 */
import { isSettledTurn, TURN_STATE } from "./turn-state.mjs";
...
const commandContract = Object.freeze({
  [TURN_TERMINAL_COMMAND.COMPLETED]: [TURN_TERMINAL_STATUS.COMPLETED, TURN_TERMINAL_REASON.RUN_COMPLETED],
  [TURN_TERMINAL_COMMAND.USER_STOPPED]: [TURN_TERMINAL_STATUS.USER_STOPPED, TURN_TERMINAL_REASON.USER_STOP],
...
function normalizeError(error = null) {
  ...
  return Object.fromEntries(Object.entries({ name: clean(error.name), message: clean(error.message || error.error), code: clean(error.code), stack: clean(error.stack) }).filter(([, value]) => value));
}
...
export function materializeTurnTerminalMessages({
  ...
  if (dialogProcessId && scopedMessages.some((message) => {
  ...
  const nextMessages = source.map((message) => (
    clean(message?.turnScopeId) === turnScopeId && clean(message?.role).toLowerCase() === "assistant"
  ...
  if (!hasAssistant && assistantMessage && typeof assistantMessage === "object" && !Array.isArray(assistantMessage)) {
    const content = String(assistantMessage.content || "");
    const messageUid = clean(assistantMessage.messageUid);
    if (content && !messageUid) {
      return Object.freeze({ materialized: false, reason: "assistant_message_identity_incomplete" });
    }
    if (content) {
      ...
      if (assistantTurnScopeId !== turnScopeId || (dialogProcessId && assistantDialogProcessId !== dialogProcessId)) {
        return Object.freeze({ materialized: false, reason: "assistant_message_identity_conflict" });
      }
      nextMessages.push({
        ...assistantMessage,
        role: "assistant",
...
export function deriveTerminalStatusFromTurn(turn = {}) {
  ...
  const command = turn.state === TURN_STATE.COMPLETED ? TURN_TERMINAL_COMMAND.COMPLETED
    : turn.state === TURN_STATE.STOP_COMPLETED ? TURN_TERMINAL_COMMAND.USER_STOPPED
      : TURN_TERMINAL_COMMAND.ERROR;
  return createTurnTerminalStatus(command, { ...turn, error: turn.failure, updatedAt: turn.updatedAt });
}

export function projectTurnTerminalStatuses(lifecycle = {}) {
  return Object.values(lifecycle?.turns || {}).map(deriveTerminalStatusFromTurn).filter(Boolean);
}
