...
export function createSessionCommandResult({ commandId, applied, deduplicated = false, aggregateVersion, emittedEventIds = [] } = {}) {
  return Object.freeze({
    ...
    emittedEventIds: Object.freeze((Array.isArray(emittedEventIds) ? emittedEventIds : []).map(clean).filter(Boolean)),
  });
}

export function validateSessionCommandResult(result = {}) {
  const errors = [];
  if (Number(result.protocolVersion) !== SESSION_PROTOCOL_VERSION) errors.push("unsupported_protocol_version");
  ...
  if (!Number.isInteger(result.aggregateVersion) || result.aggregateVersion < 0) errors.push("invalid_aggregate_version");
