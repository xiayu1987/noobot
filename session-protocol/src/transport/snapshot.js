...
export function validateSessionSnapshot(snapshot = {}) {
  ...
  if (Number(snapshot.protocolVersion) !== SESSION_PROTOCOL_VERSION)
    errors.push("unsupported_protocol_version");
  if (!Number.isInteger(snapshot.aggregateVersion) || snapshot.aggregateVersion < 0)
    errors.push("invalid_aggregate_version");
  if (!Number.isInteger(snapshot.throughSequence) || snapshot.throughSequence < 0)
    errors.push("invalid_through_sequence");
  if (!snapshot.session || typeof snapshot.session !== "object" || Array.isArray(snapshot.session))
    errors.push("invalid_session");
  else if (
    String(snapshot.session.sessionId || "").trim() !==
    String(snapshot.scope?.sessionId || "").trim()
  )
    errors.push("session_identity_mismatch");
