...
test("session log registries reference controls with defaults and environment keys", () => {
  ...
  for (const [debugType, descriptor] of Object.entries(RUNTIME_EVENTS_SESSION_LOG_DEBUG_TYPES)) {
    assert.equal(
      typeof descriptor.exposeToClient,
      "boolean",
      `${debugType} should declare client exposure`,
    );
    assert.equal(
      typeof RUNTIME_EVENTS_CONFIG_DEFAULTS.sessionLogControls.debug[descriptor.controlKey],
      "boolean",
    );
    assert.equal(
      typeof RUNTIME_EVENTS_CONFIG_ENVS.sessionLogControls.debug[descriptor.controlKey],
      "string",
    );
  }
});
...
test("runtime-events storage config uses shared defaults", () => {
  assert.deepEqual(
    resolveRuntimeEventsStorageConfig({}),
    RUNTIME_EVENTS_CONFIG_DEFAULTS.runtimeEvents,
  );
});
...
test("execution log controls are centralized and suppress full successful turn diagnostics by default", () => {
  ...
  assert.equal(
    shouldRecordRuntimeExecutionLog({ event: "assistant_message_saved" }, { env: {} }),
    false,
  );
  assert.equal(
    shouldRecordRuntimeExecutionLog({ event: "tool_message_saved" }, { env: {} }),
    false,
  );
  assert.equal(
    shouldRecordRuntimeExecutionLog(
...
test("hook runtime-events mode defaults to summary and recognizes verbose values", () => {
  ...
  for (const value of ["verbose", "trace", "debug", "full", "1", "true", "on", "yes"]) {
    assert.equal(
      isHookRuntimeEventVerboseEnabled({
        env: { [RUNTIME_EVENTS_CONFIG_ENVS.hookRuntimeEvents.mode]: value },
      }),
      true,
    );
  }
