...
const desktopTargetArch =
  process.env.NOOBOT_DESKTOP_ARCH || process.env.npm_config_arch || process.arch;
...
async function main() {
  ...
  log(
    `Node=${process.version}; platform=${process.platform}; arch=${process.arch}; targetArch=${desktopTargetArch}`,
  );
  ...
  const servicePkg = JSON.parse(
    await readFile(path.join(repoRoot, "service/package.json"), "utf8"),
  );
  const agentPkg = JSON.parse(await readFile(path.join(repoRoot, "agent/package.json"), "utf8"));
  const desktopPkg = JSON.parse(
    await readFile(path.join(desktopProjectDir, "package.json"), "utf8"),
  );
  await writeFile(
  ...
  const ripgrepPackages = getDesktopRipgrepPackages(
    desktopPkg.name,
    ripgrepVersion,
    desktopTargetArch,
  );
  if (!ripgrepPackages.length) {
    ...
    await Promise.all(
      ripgrepPackages.map((packageSpec) =>
        assertExists(
          path.join(backendRoot, getRipgrepBinaryRelativePath(packageSpec)),
          `Bundled ripgrep binary ${packageSpec}`,
        ),
      ),
    );
  }
  ...
  await assertExists(
    path.join(backendRoot, "agent/src/prompts/base.md"),
    "Prepared backend system prompt",
  );
  await assertExists(
    path.join(backendRoot, "agent/src/prompts/base.zh-CN.md"),
    "Prepared backend Chinese system prompt",
  );
  await assertExists(
