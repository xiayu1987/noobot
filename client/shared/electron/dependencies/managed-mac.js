...
export function createMacDependencyInstallerTools({
  ...
  function getProxyOptions() {
    ...
    return {
      proxyUrl,
      args: getCurlProxyArgs(proxyUrl),
  ...
  async function fetchLibreOfficeStableVersions() {
    ...
    writeDependencyLog("proxy:curl", {
      enabled: Boolean(proxy.proxyUrl),
      proxy: proxy.masked,
      target: "libreoffice-index",
    });
    ...
    writeDependencyLog("dmg:versions:finish", {
      ok: result.ok,
      code: result.code,
      error: result.error,
    });
  ...
  function findLibreOfficeAppInVolume(volumePath) {
    ...
    try {
      ...
      const match = entries.find(
        (entry) => entry.isDirectory() && entry.name.toLowerCase() === "libreoffice.app",
      );
  ...
  async function downloadFileWithCurl(
    url,
    destinationPath,
    { timeoutMs, label = "file", eventPrefix = "download" } = {},
  ) {
    ...
    const result = await runProcess(
      ...
      { timeoutMs, env: proxy.env },
    );
    writeDependencyLog(`${eventPrefix}:finish`, {
      ok: result.ok,
      code: result.code,
      error: result.error,
    });
  ...
  async function downloadFirstAvailableLibreOfficeDmg(spec, destinationPath) {
    ...
    const failures = [];
    writeDependencyLog("dmg:download:candidates", {
      count: candidates.length,
      urls: candidates.join(" | "),
    });
    for (const url of candidates) {
      try {
        await fs.promises.rm(destinationPath, { force: true });
        await downloadFileWithCurl(url, destinationPath, {
          timeoutMs: desktopDependencyTimeouts.downloadMs,
          label: "LibreOffice DMG",
          eventPrefix: "dmg:download",
        });
        return url;
      } catch (error) {
        ...
        await runBestEffort(() => fs.promises.rm(destinationPath, { force: true }), {
          operationName: "macDependencies.removeFailedArchive",
          context: { destinationPath },
        });
      }
    }
    throw createDependencyError(
      `Failed to download LibreOffice DMG from official candidates. Tried: ${failures.join(" ; ")}. You can set NOOBOT_LIBREOFFICE_MAC_DMG_URL to a verified LibreOffice macOS DMG URL, or install manually from https://www.libreoffice.org/download/download-libreoffice/`,
      { failureKind: "download", retryable: true },
    );
  }
  ...
  async function installLibreOfficeFromDmg(spec) {
    ...
    try {
      sendStatus({
        phase: "dependency",
        message: `Downloading ${spec.label} from the official LibreOffice site...`,
      });
      ...
      writeDependencyLog("dmg:attach:start", {
        path: dmgPath,
        timeoutMs: desktopDependencyTimeouts.dmgAttachMs,
      });
      const attachResult = await runProcess(
        "hdiutil",
        ["attach", dmgPath, "-nobrowse", "-readonly"],
        { timeoutMs: desktopDependencyTimeouts.dmgAttachMs },
      );
      writeDependencyLog("dmg:attach:finish", {
        ok: attachResult.ok,
      ...
      if (!attachResult.ok) {
        const detail = String(
          attachResult.stderr || attachResult.stdout || attachResult.error || "",
        )
          .trim()
          .slice(0, 1000);
        throw createDependencyError(
          `Failed to mount LibreOffice DMG.${detail ? ` ${detail}` : ""}`,
          { failureKind: "package" },
        );
      }
      mountPoint = parseHdiutilMountPoint(
        `${attachResult.stdout || ""}\n${attachResult.stderr || ""}`,
      );
      writeDependencyLog("dmg:mount-point", { path: mountPoint });
      if (!mountPoint)
        throw createDependencyError("Failed to locate LibreOffice DMG mount point.", {
          failureKind: "package",
        });
      ...
      if (!sourceApp)
        throw createDependencyError("Mounted LibreOffice DMG did not contain LibreOffice.app.", {
          failureKind: "package",
        });

      const targetApp = "/Applications/LibreOffice.app";
      sendStatus({ phase: "dependency", message: `Copying ${spec.label} to /Applications...` });
      writeDependencyLog("dmg:copy:start", {
        source: sourceApp,
        target: targetApp,
        timeoutMs: desktopDependencyTimeouts.appCopyMs,
      });
      const copyResult = await runProcess("ditto", [sourceApp, targetApp], {
        timeoutMs: desktopDependencyTimeouts.appCopyMs,
      });
      writeDependencyLog("dmg:copy:finish", {
        ok: copyResult.ok,
        code: copyResult.code,
        error: copyResult.error,
      });
      if (!copyResult.ok) {
        const detail = String(copyResult.stderr || copyResult.stdout || copyResult.error || "")
          .trim()
          .slice(0, 1000);
        throw createDependencyError(
          `Failed to copy LibreOffice to /Applications. macOS may require permission to write to /Applications.${detail ? ` ${detail}` : ""}`,
      ...
      if (mountPoint) {
        ...
        writeDependencyLog("dmg:detach:start", {
          mount: mountPoint,
          timeoutMs: desktopDependencyTimeouts.dmgDetachMs,
        });
        const detachResult = await runProcess("hdiutil", ["detach", mountPoint], {
          timeoutMs: desktopDependencyTimeouts.dmgDetachMs,
        });
        writeDependencyLog("dmg:detach:finish", {
          ok: detachResult.ok,
          code: detachResult.code,
          error: detachResult.error,
        });
      }
      void runBestEffort(() => fs.promises.rm(tempDir, { recursive: true, force: true }), {
        operationName: "macDependencies.removeNodeTemporaryDirectory",
        context: { tempDir },
      });
    }
  }
  ...
  async function runManagedCommand(key, command, args = []) {
    ...
    if (!commandPath || !hasExistingFile(commandPath))
      return { ok: false, error: "managed command missing" };
  ...
  async function downloadFirstAvailableFile({
    key,
    label,
    candidates,
    destinationPath,
    ...
    const failures = [];
    writeDependencyLog(`${eventPrefix}:candidates`, {
      key,
      count: candidates.length,
      urls: candidates.join(" | "),
    });
    for (const url of candidates) {
      try {
        ...
        const message = error?.message || String(error);
        failures.push(`${url} => ${message.slice(0, 500)}`);
        writeDependencyLog(`${eventPrefix}:candidate-failed`, {
          key,
          url,
          error: message.slice(0, 1000),
        });
        await runBestEffort(() => fs.promises.rm(destinationPath, { force: true }), {
          operationName: "macDependencies.removeFailedBrowserArchive",
          context: { destinationPath },
        });
      }
    }
    throw createDependencyError(
      `Failed to download ${label}. Tried: ${failures.join(" ; ")}.${envHint ? ` You can set ${envHint}.` : ""}${manualUrl ? ` Manual download: ${manualUrl}` : ""}`,
      { failureKind: "download", retryable: true },
    );
  }
  ...
  async function extractFfmpegArchive({ archivePath, extractDir, selectedUrl }) {
    if (isGzipArchiveUrl(selectedUrl)) {
      ...
      writeDependencyLog("managed:ffmpeg:extract:start", {
        archive: archivePath,
      ...
      writeDependencyLog("managed:ffmpeg:extract:finish", {
        ok: true,
        format: "gzip",
        destination: outputPath,
      });
    ...
    const unzipResult = await runProcess("ditto", ["-x", "-k", archivePath, extractDir], {
      timeoutMs: desktopDependencyTimeouts.installMs,
    });
    writeDependencyLog("managed:ffmpeg:extract:finish", {
      ok: unzipResult.ok,
      code: unzipResult.code,
      error: unzipResult.error,
      stderr: String(unzipResult.stderr || "").slice(0, 500),
      format: "zip",
    });
    if (!unzipResult.ok)
      throw createDependencyError(
        `Failed to extract FFmpeg.${String(unzipResult.stderr || unzipResult.error || "").slice(0, 1000)}`,
        { failureKind: "package" },
      );
  }
  ...
  async function installFfmpegManagedMac(spec) {
    ...
    try {
      ...
      if (!sourcePath)
        throw createDependencyError("FFmpeg archive did not contain ffmpeg binary.", {
          failureKind: "package",
        });
      sendStatus({
        phase: "dependency",
        message: `Installing ${spec.label} to managed dependencies...`,
      });
      ...
      writeDependencyLog("managed:ffmpeg:copy:finish", {
        target: targetPath,
        mode: getFileMode(targetPath),
      });
      ...
      if (!verify.ok)
        throw createDependencyError(
          `Managed FFmpeg verification failed at ${verify.path || targetPath}. exists=${verify.exists}; mode=${verify.mode}; ${String(verify.stderr || verify.error || "").slice(0, 1000)}`,
          { failureKind: "verification" },
        );
      return { ok: true, method: "managed", path: verify.path };
    } finally {
      void runBestEffort(() => fs.promises.rm(tempDir, { recursive: true, force: true }), {
        operationName: "macDependencies.removeBrowserTemporaryDirectory",
        context: { tempDir },
      });
    }
  }
  ...
  async function installNodeManagedMac(spec) {
    ...
    try {
      ...
      const tarResult = await runProcess(
        "tar",
        ["-xJf", archivePath, "-C", extractDir, "--strip-components", "1"],
        { timeoutMs: desktopDependencyTimeouts.installMs },
      );
      if (!tarResult.ok)
        throw createDependencyError(
          `Failed to extract Node.js.${String(tarResult.stderr || tarResult.error || "").slice(0, 1000)}`,
          { failureKind: "package" },
        );
      ...
      await runBestEffort(
        () => fs.promises.chmod(joinClientPath(targetDir, "bin", "node"), 0o755),
        { operationName: "macDependencies.makeNodeExecutable", context: { targetDir } },
      );
      await runBestEffort(() => fs.promises.chmod(joinClientPath(targetDir, "bin", "npm"), 0o755), {
        operationName: "macDependencies.makeNpmExecutable",
        context: { targetDir },
      });
      prependManagedDependencyPath();
      ...
      if (!nodeVerify.ok)
        throw createDependencyError(
          `Managed Node.js verification failed.${String(nodeVerify.stderr || nodeVerify.error || "").slice(0, 1000)}`,
          { failureKind: "verification" },
        );
      ...
      void runBestEffort(() => fs.promises.rm(tempDir, { recursive: true, force: true }), {
        operationName: "macDependencies.removeNpmTemporaryDirectory",
        context: { tempDir },
      });
    }
  }
