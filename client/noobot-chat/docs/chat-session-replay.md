# Chat Session / Replay 状态与流程说明（已过期）

> ⚠️ **文档状态：已过期（Deprecated）**  
> 生效日期：**2026-05-15**  
...
- 用户手动切换：应继续拦截并提示“消息发送中，已保持当前会话”。
- 内部恢复切换（reconnect/fetch 恢复链路）：应使用 `silent: true`，允许切换，不要弹该提示。

否则刷新恢复时会出现“消息发送中不允许切换”这类噪声告警，虽然不影响最终渲染，但会干扰排障。
...
7. `conversationStates`（agent-proxy 状态快照）先用于恢复发送/终态，再叠加 replay 消息。
...
### 4.4 缺少当轮 user 时，先渲染 session 再 replay

多端并发场景下，当前端收到某轮 `DELTA/THINKING` 时，可能只看到了 assistant 增量，还没拿到该轮 user（常见于另一端发起请求，本端先收到回放流）。

当前约束：

1. 若 `dialogProcessId` 对应 assistant 不存在；
2. 且当前会话里没有可复用的 pending assistant；
3. 且当前消息尾部也不是 user；
4. 且回放批次是 in-flight 事件（`DELTA/THINKING/INTERACTION_REQUEST`）；

则先执行一次 session hydrate（`fetchSessionDetail + applySessionDetail`），再继续回放，避免出现“先显示 AI、user 要刷新才出现”。

实现点：

- `useReconnectReplay.js`
  - `shouldHydrateSessionBeforeReplay()`
  - `renderActiveSessionBeforeReplay()`
  - `replayHydrationPromise`（防并发重复 hydrate）
...
回放批次如果仍是“进行中”（没有 terminal 事件，但有 `DELTA/THINKING/INTERACTION_REQUEST`），应恢复 `sending=true`，避免 UI 丢失“发送中”状态。

约束：

- 非 terminal batch + in-flight event -> `sending.value = true`
- terminal（`DONE/STOPPED/ERROR`）仍由终态清理统一置回 false
- 若收到 `channel_state`（如 `sending/completed/stopped/error/no_conversation`），前端优先按状态更新 `sending` 与交互态，再处理消息体 patch。
- `channel_state` 额外状态：
  - `reconnecting`：重连中，前端保持 sending=true，避免短暂误判空闲。
  - `stopping`：已请求停止但未收到终态，前端仍视为进行中。
  - `expired`：回放缓存已过期，前端触发静默刷新并重建会话快照。
...
从 2026-05-15 起，前端会话 UI 状态统一由 `channel_state` 驱动（包括实时流和回放），事件本身只负责内容数据（`delta/thinking/done.messages` 等）。

### 9.1 状态列表与处理语义

| 状态 | 含义 | 前端处理 |
| --- | --- | --- |
| `no_conversation` | 当前会话无进行中对话 | `sending=false`，清理 pending interaction |
| `sending` | 正在流式输出 | `sending=true`，assistant `pending=true`，清理同轮过期交互态 |
| `interaction_pending` | 等待用户交互输入 | `sending=true`，`interactionSubmitting=true`，恢复 `pendingInteractionRequest`（来自 `pendingInteraction`） |
| `stopping` | 已请求停止，等待终态 | `sending=true`，assistant `statusLabel=stopping` |
| `reconnecting` | 重连恢复中 | `sending=true`，assistant `statusLabel=reconnecting` |
| `completed` | 后端正常完成，非前端终态 | 前端仍视为 in-flight：`sending=true`，等待前端 completion 收敛 |
| `frontend_completion_requesting` | 前端正在收敛 completion | `sending=true`，assistant 暂不标记完成 |
| `frontend_completed` | 前端完成终态 | `sending=false`，assistant `pending=false`，`statusLabel=generated`，清理同轮交互态 |
| `stopped` | 用户或链路停止 | `sending=false`，assistant `pending=false`，`statusLabel=stopped`，清理同轮交互态 |
| `error` | 失败终态 | `sending=false`，assistant `pending=false`，`statusLabel=failed`，清理同轮交互态 |
| `expired` | replay 缓存过期 | `sending=false`，清理交互态，触发静默会话刷新 |

> 注：`ERROR` 事件里的 `error` 文本仍会写入消息对象，但不会直接驱动 UI 状态；UI 状态收敛以 `channel_state:error` 为准。

### 9.2 事件与状态分离

- **事件层（内容）**：`delta` 追加文本、`thinking` 追加日志、`done.messages` patch 消息快照。
- **状态层（状态机）**：只处理 `channel_state`，负责 `sending/pending/statusLabel/交互态`。
...
```txt
send
  -> stream events(delta/thinking/...)
  -> channel_state:sending / interaction_pending / ...
  -> channel_state:completed (后端完成，非前端终态)
  -> frontend_completion_requesting -> frontend_completed
  -> channel_state:stopped|error
  -> UI 状态收敛
```
...
```txt
reconnect
  -> reconnect_data.sessions[].conversationStates (状态快照先到)
  -> applyChannelState(...) 恢复 sending/interaction 状态
  -> replay dialog events (只补内容)
  -> 后续 channel_state 继续驱动终态收敛
```

### 9.5 交互态一致性（多端/刷新）

agent-proxy 在 `interaction_pending` 的 `channel_state` / `conversationStates` 中附带：
...
前端据此恢复 `pendingInteractionRequest`，因此刷新或新端加入时交互弹窗状态保持一致。

## 10. 状态机调试面板

为排查“刷新后状态变化 / 多端不一致”，新增可选调试面板（默认关闭）：

- 环境变量：`VITE_NOOBOT_DEBUG_CONVERSATION_STATE_PANEL=true`
- 位置：主聊天区底部（composer 下方）
- 展示：
  - `sending` / `interactionSubmitting` / 当前 pending request
  - `State Snapshot`（每个 `sessionId::dialogProcessId` 当前状态）
  - `State Timeline`（最近 80 条状态事件，含 source/seq/applied）

其中 `applied=false` 表示该状态不是当前激活会话，前端记录但不应用到 UI。
...
- 当回放收到同一轮的非交互事件（如 `DELTA/THINKING/DONE`）时，清理已过期的 pending interaction，避免“另一端已确认，本端仍停留在交互态”。
...
- [ ] 发送中切会话是否区分了“用户手动切换（拦截）”和“内部 silent 恢复切换（放行）”？
...
- [ ] 多端回放若缺少当轮 user，是否先 hydrate session 再 replay？
- [ ] 回放中（DELTA/THINKING/INTERACTION_REQUEST）是否恢复 `sending=true`？
...
applyReconnectMessagesToActiveSession:
  ...
  whether pre-hydrate triggered, sending before/after
