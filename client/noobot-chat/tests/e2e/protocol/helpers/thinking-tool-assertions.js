...
export function assertCanonicalToolPairs(events = [], expectedToolNames = []) {
  ...
  expect(calls.map((event) => text(event.data?.tool)).sort()).toEqual([...expectedToolNames].sort());
...
export async function observeRealtimeThinkingChanges(page) {
  ...
  }, { timeout: 120000, intervals: [100, 200, 500] }).toBeGreaterThanOrEqual(2);
  return shell;
}
...
export async function assertRealtimeToolDetails(shell, expectedLineCount) {
  ...
  await expect(toolLines.locator(".base-thinking-log-line__event", { hasText: "调用" }))
    .toHaveCount(expectedLineCount / 2);
  await expect(toolLines.locator(".base-thinking-log-line__event", { hasText: "返回" }))
    .toHaveCount(expectedLineCount / 2);
...
export async function readRealtimeToolProjection(page) {
  ...
  for (let index = 0; index < await toolLines.count(); index += 1) {
    ...
    const expandable = await trigger.evaluate((element) => element.classList.contains("is-expandable"));
    let detail = "";
    if (expandable) {
      await trigger.scrollIntoViewIfNeeded();
      await trigger.click();
...
export async function waitForToolSet(userId, sessionId, turnScopeId, expectedToolNames) {
  return waitForSessionExecutionEventTree(userId, sessionId, (records) => {
    const events = toolEventsForTurn(records, turnScopeId);
    const calls = events.filter((event) => event.event === "tool_call_start");
    const results = events.filter((event) => event.event === "tool_call_end");
    return calls.length === expectedToolNames.length && results.length === expectedToolNames.length;
  }, { timeoutMs: 240000 });
}
