...
const configuredWorkspaceRoots = [
  workspaceRoot,
  e2eWorkspaceRoot,
  runtimeEventsWorkspaceRoot,
].filter(Boolean);

function resolveBaseUrl(value) {
  const raw = String(value || "").trim();
  const topology = resolveRuntimeTopology(process.env);
  const candidate = raw || `http://${topology.loopbackHost}:${addressPort(topology.clientAddr)}`;
  let parsed;
  try {
...
export default defineConfig({
  ...
  reporter: [
    ["line"],
    [path.join(protocolRoot, "suite-session-cleanup.js")],
    [
      "html",
      {
        open: "never",
