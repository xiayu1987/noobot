...
test("@core PBE-039 agent config protocol keeps selected scenario through a real turn", async ({
  noobot,
  protocolCapture,
}, testInfo) => {
  ...
  await sendMessage(
    noobot.page,
    uniquePrompt(testInfo, "Reply exactly CONFIG-PROTOCOL-OK without calling a tool"),
  );
