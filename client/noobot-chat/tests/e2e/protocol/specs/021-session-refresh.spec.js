...
test("@core PBE-021 自然完成后刷新 Session 并继续发送", async ({
  noobot,
  protocolCapture,
}, testInfo) => {
  await sendMessage(
    noobot.page,
    uniquePrompt(testInfo, "reply with exactly ACK-1 and do not use tools"),
  );
  ...
  const count = commandsForSession(protocolCapture, noobot.sessionId).filter(
    (item) => item.commandType === "turn.send",
  ).length;
  await reloadAndWaitForReconnect(noobot.page, protocolCapture);
  expect(
    commandsForSession(protocolCapture, noobot.sessionId).filter(
      (item) => item.commandType === "turn.send",
    ),
  ).toHaveLength(count);
  ...
  await sendMessage(
    noobot.page,
    uniquePrompt(testInfo, "reply with exactly ACK-2 after refresh and do not use tools"),
  );
  const secondSend = await waitForCommand(protocolCapture, noobot.sessionId, "turn.send", count);
  expect(secondSend.concurrency.expectedAggregateVersion).toBeGreaterThan(
    send.concurrency.expectedAggregateVersion,
  );
  await waitForNaturalCompletion({
    page: noobot.page,
    capture: protocolCapture,
...
test("@core PBE-022 停止后关闭浏览器再打开并继续", async ({
  noobot,
  protocolCapture,
}, testInfo) => {
  ...
  const { send } = await sendAndStop({
    ...
    capture: protocolCapture,
    sessionId: noobot.sessionId,
    prompt: uniquePrompt(testInfo, "stop before browser restart"),
    attachment: file,
  });
  ...
  const continued = await continueAndStop({
    ...
    sessionId: noobot.sessionId,
    previous: send,
    prompt: uniquePrompt(testInfo, "continue after browser restart"),
  });
