...
test("@full PBE-034 主流程低轮次 task_summary checkpoint 与模型输入闭环", async ({
  noobot,
  protocolCapture,
}, testInfo) => {
  ...
  const records = await waitForSessionExecutionEventTree(
    ...
    (items) => {
      ...
      return (
        scoped.some((item) => item.event === "phase_summary_required") &&
  ...
  for (const messageUid of summarizedIds)
    expect(messagesByUid.get(messageUid)?.summarized, messageUid).toBe(true);
  ...
  const phasePrompts = messages.filter(
    (message) => resolveContextInternalMessageType(message) === "noobot.phase_summary_prompt",
  );
  ...
  const taskSummaryResults = messages.filter(
    (message) => toolResultName(message) === "task_summary",
  );
  ...
  const summaryExchanges = taskSummaryCalls.map(({ call }) => {
    ...
    const resultMessage = taskSummaryResults.find(
      (message) => String(message.tool_call_id || "").trim() === toolCallId(call),
    );
    ...
    const attachmentRefs = (resultMessage.transferEnvelopes || [])
      ...
      .filter(
        (attachment) =>
          (attachment?.descriptor?.name || attachment?.name) ===
          "task-summary-content.tool-input.md",
      );
    expect(attachmentRefs.length).toBeGreaterThan(0);
    expect(
      new Set(attachmentRefs.map((attachment) => attachment?.identity?.attachmentId)).size,
    ).toBe(1);
    return {
      fullSummaryContent,
      resultMessage,
  ...
  for (const exchange of summaryExchanges) {
    const persistedSummaryAttachments = modelAttachments.filter(
      (item) =>
    ...
    expect(
      await fs.readFile(
        path.join(workspaceRoot(), noobot.userId, persistedSummaryAttachments[0].storageRef.ref),
        "utf8",
      ),
    ).toBe(exchange.fullSummaryContent);
  }
  ...
  for (const exchange of summaryExchanges) {
    expect(
      Object.values(refreshedModelAttachmentIndex.attachments || {}).some(
        (item) =>
          item.identity?.attachmentId === exchange.summaryAttachmentId &&
          item.descriptor?.name === "task-summary-content.tool-input.md",
      ),
  ...
  const observedCheckpointRevisions = new Set(
    mainInvocations.map((invocation) => invocation.data.context.summaryCheckpointRevision),
  );
  ...
  for (const phasePrompt of phasePrompts) {
    expect(
      mainInvocations.some((invocation) =>
        invocation.data.messages.preview.some(
          (message) => message.messageId === phasePrompt.messageUid,
        ),
      ),
    ).toBe(true);
  }
  for (const receipt of receipts) {
    const afterReceipt = mainInvocations.filter(
      (record) =>
    ...
    for (const invocation of afterReceipt) {
      ...
      const visibleIds = new Set(
        invocation.data.messages.preview.map((message) => message.messageId).filter(Boolean),
      );
  ...
  expect(toolResultNames.filter((name) => name === "task_summary")).toHaveLength(
  ...
  const secondTurnRecords = await waitForSessionExecutionEventTree(
    ...
    noobot.sessionId,
    (items) => {
      const scopedSecondTurn = items.filter(
