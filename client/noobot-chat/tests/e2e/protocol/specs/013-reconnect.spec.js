...
test("@core PBE-013 运行中刷新后执行记录可展开并收敛到终态", async ({
  ...
  await waitForSessionExecutionEventTree(noobot.userId, noobot.sessionId, (records) =>
    records.some(
      (record) =>
  ...
  await expect(liveShell.locator(".thinking-detail-action-button")).toHaveText(
    /Thinking Details \([1-9]\d*\)/,
  );
  ...
  const detailsDrawer = noobot.page
    .locator(".el-drawer")
    .filter({ has: noobot.page.locator(".thinking-details-panel") })
    .last();
  ...
  await expect(
    detailsDrawer.locator(".thinking-details-panel .base-thinking-log-line__detail"),
  ).toHaveCount(1);
  await runningToolLine.locator(".base-thinking-log-line__text").click();
  await expect(
    detailsDrawer.locator(".thinking-details-panel .base-thinking-log-line__detail"),
...
test("@core PBE-014 reconnect 与新 Continue 并发", async ({
  noobot,
  protocolCapture,
}, testInfo) => {
  ...
  await noobot.page
    ...
    .fill(uniquePrompt(testInfo, "continue during reconnect"));
