    ...
    timeline.map((item) => ({
      ...
      ...(item.call
        ? {
            call: {
              ...item.call,
              ...(item.call.log ? { log: { ...item.call.log, ts: "<volatile>" } } : {}),
            },
          }
        : {}),
      ...(item.resultEvent
        ? {
            resultEvent: {
              ...
              ...(item.resultEvent.log
                ? { log: { ...item.resultEvent.log, ts: "<volatile>" } }
                : {}),
            },
          }
        : {}),
...
describe("turnProjectionStore convergence", () => {
  ...
  it("hydrates idempotently, replays buffered increments, and excludes Turn UI state", () => {
    ...
    const snapshotBase = replay(
      message(),
      events.slice(0, 2),
      TURN_PROJECTION_SOURCE.HISTORY_REPLAY,
    );
    const snapshot = {
    ...
    expect(
      hydrateTurnSnapshot({
        targetMessage: target,
        snapshot: structuredClone(target),
        throughSequence: 3,
      }).applied,
    ).toBe(true);
