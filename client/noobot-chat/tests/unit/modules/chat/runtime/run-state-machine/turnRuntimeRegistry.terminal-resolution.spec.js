...
it("settles a same-revision terminal snapshot even when its execution state is still sending", () => {
  ...
  const hydrated = applyTurnLifecycleSnapshot(
    registry,
    snapshot({
      sessionId: "s-refresh",
      sequence: 4,
...
it.each([
  ...
  "allows only the matching authoritative success to settle a retryable $name failure",
  ({ prepare, failedPhase, failedState, successEvent, terminal }) => {
    const registry = createTurnRuntimeRegistryState();
    prepare(registry);
    const failureTerminalState = failedPhase === "stop" ? "stop_failed" : "completion_failed";
    const failed = settleTerminal(registry, {
      state: failureTerminalState,
      revision: 6,
      sequence: 6,
      failure: { phase: failedPhase, message: "retryable" },
...
it("uses the preallocated Session identity without deferred detail authority", () => {
  ...
  expect(
    settleTerminal(registry, { sessionId: "backend-session", revision: 4, sequence: 4 }).applied,
  ).toBe(true);
});
...
it("completes a normal request when final detail carries a different dialog route", () => {
  ...
  const completed = settleTerminal(registry, {
