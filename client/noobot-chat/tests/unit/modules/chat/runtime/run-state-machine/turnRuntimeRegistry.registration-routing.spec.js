...
describe("turnRuntimeRegistry: registration and routing", () => {
  it("indexes canonical Execution Turn identities per Session and removes only the targeted Session", () => {
    ...
    expect(
      applyExecutionSnapshot(registry, execution("e1", "s1", "workflow-node:shared")).applied,
    ).toBe(true);
    expect(
      applyExecutionSnapshot(registry, execution("e2", "s2", "workflow-node_shared")).applied,
    ).toBe(true);
    expect(registry.executionIdByTurnScopeId[executionTurnKey("s1", "workflow-node_shared")]).toBe(
      "e1",
    );
    expect(registry.executionIdByTurnScopeId[executionTurnKey("s2", "workflow-node:shared")]).toBe(
      "e2",
    );
    ...
    expect(
      registry.executionIdByTurnScopeId[executionTurnKey("s1", "workflow-node_shared")],
    ).toBeUndefined();
    expect(registry.executionIdByTurnScopeId[executionTurnKey("s2", "workflow-node_shared")]).toBe(
      "e2",
    );
  });
  ...
  it.each([BackendChannelState.RECONNECTING, BackendChannelState.INTERACTION_PENDING])(
    "keeps %s action-locked but rejects stop without mutating the turn",
    (state) => {
      const registry = createTurnRuntimeRegistryState();
      sendStart(registry, { sessionId: "s1", turnScopeId: "t1" });
      backendState(registry, {
  ...
  it("rejects a mismatched dialog identity without mutating the Turn", () => {
    ...
    const dialogMismatch = backendState(registry, {
      sessionId: "s1",
      turnScopeId: "t1",
    ...
    expect(dialogMismatch).toMatchObject({
      applied: false,
      reason: "dialog_process_identity_conflict",
    });
  ...
  it("isolates identical canonical Turn scopes by Session", () => {
    ...
    backendState(registry, {
      sessionId: "s1",
      turnScopeId: "workflow-node_shared",
      dialogProcessId: "dp1",
      state: BackendChannelState.SENDING,
      seq: 2,
    });
    backendState(registry, {
      sessionId: "s2",
      turnScopeId: "workflow-node:shared",
      dialogProcessId: "dp2",
      state: BackendChannelState.SENDING,
      seq: 2,
    });
  ...
  it("exposes message runtime only for the owning session and turn", () => {
    ...
    expect(
      selectTurnMessageRuntime(registry, { sessionId: "s1", turnScopeId: "t1" }),
    ).toMatchObject({
      sessionId: "s1",
      turnScopeId: "t1",
      dialogProcessId: "dp1",
    ...
    expect(
      selectTurnMessageRuntime(registry, { sessionId: "s1", dialogProcessId: "dp1" }),
    ).toMatchObject({ turnScopeId: "t1" });
    expect(selectTurnMessageRuntime(registry, { sessionId: "s2", turnScopeId: "t1" })).toBeNull();
    expect(
      selectTurnMessageRuntime(registry, {
        sessionId: "s1",
        turnScopeId: "t1",
        dialogProcessId: "other",
      }),
    ).toMatchObject({
      sessionId: "s1",
      turnScopeId: "t1",
