...
export function lifecycle(
  registry,
  {
    ...
    revision = 2,
    sequence = 2,
    canStop = state === "processing",
    action = "send",
    commandId = "command-1",
  } = {},
) {
  lifecycleSequence += 1;
  return applyTurnLifecycleEnvelope(
    registry,
    createTurnLifecycleEnvelope({
      eventType,
...
export function settleTerminal(
  registry,
  {
    sessionId = "s1",
    turnScopeId = "t1",
  ...
  const terminalFailure = state.endsWith("_failed")
    ? failure || {
        ...
        message: `${state} terminal failure`,
        retryable: false,
      }
    : failure;
...
export function snapshot(overrides = {}) {
  const withMessageIdentity = (turn) =>
    turn
      ? {
      ...
      : null;
  const activeTurn = withMessageIdentity(
  ...
  const recentTerminalTurns = (
    Array.isArray(overrides.recentTerminalTurns) ? overrides.recentTerminalTurns : []
  ...
  return {
    ...
    sessionId: "s1",
    sequence: 2,
    activeTurnScopeId: activeTurn?.turnScopeId || "",
    replacedTurns: [],
    unchanged: false,
