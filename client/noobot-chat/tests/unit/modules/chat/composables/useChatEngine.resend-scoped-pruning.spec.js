...
describe("useChatEngine.resend scoped pruning", () => {
  it("resendMonotonicMessage keeps edited content when reusing a stale user message object", async () => {
    ...
    const stream = vi.fn(async () => {
      observedUserMessage = activeSession.value.messages.find(
    ...
    const first = {
      turnScopeId: "client-turn:old-stale",
      role: RoleEnum.USER,
    ...
    expect(observedUserMessage).toEqual(
  ...
  it("resendMonotonicMessage uses backend replace-turn mapping to prune stale replaced messages", async () => {
    ...
    await expect(engine.resendMonotonicMessage(oldAssistant, "edited question")).resolves.toBe(
      true,
    );
    ...
    expect(
      observedMessagesAtStream.find((message) => message.id === "old-assistant"),
    ).toBeUndefined();
    expect(observedMessagesAtStream).toEqual(
    ...
    expect(activeSession.value.messages.find((message) => message.id === "new-user")).toEqual(
      expect.objectContaining({
        role: RoleEnum.USER,
        content: "edited question",
        turnScopeId: expect.stringMatching(/^client-turn:/),
  ...
  it("resendMonotonicMessage keeps previous duplicate-content turn when resending latest scoped user", async () => {
    ...
    expect(observedMessagesAtStream[2]).toEqual(
      expect.objectContaining({
        role: RoleEnum.USER,
        content: "same question",
        turnScopeId: expect.stringMatching(/^client-turn:/),
    ...
    expect(observedMessagesAtStream[3]).toEqual(
      expect.objectContaining({
        role: RoleEnum.ASSISTANT,
        content: "",
        pending: false,
  ...
  it("resendMonotonicMessage starts generation from the user-only replace-turn snapshot", async () => {
    ...
    const replaceSessionTurnApi = vi.fn(async ({ turnScopeId, commandId, anchor }) => {
      const replacementUser = {
        id: "msg-user-replace-completed",
        messageId: "msg-user-replace-completed",
        turnScopeId,
        role: RoleEnum.USER,
        content: "edited question",
      };
    ...
    expect(activeSession.value.messages[1]).toEqual(
      expect.objectContaining({
        role: RoleEnum.ASSISTANT,
        pending: false,
      }),
