...
describe("useChatEngine.resend stopped state", () => {
  ...
  it("resendMonotonicMessage rejects stale stopped replacement snapshots without the new turnScopeId", async () => {
    ...
    await expect(
      engine.resendMonotonicMessage(staleStoppedAssistant, "edited again"),
    ).resolves.toBe(false);
  ...
  it("resendMonotonicMessage can repeatedly replace a stopped turn and append a fresh assistant placeholder", async () => {
    ...
    await expect(engine.resendMonotonicMessage(stoppedAssistant, "second resend")).resolves.toBe(
      true,
    );
  ...
  it("can stop and edit-resend twice through persisted USER_STOPPED events", async () => {
    ...
    await expect(
      engine.resendMonotonicMessage(activeSession.value.messages[1], "first edit"),
    ).resolves.toBe(true);
    const firstStoppedAssistant = activeSession.value.messages.find(
      (message) => message.role === RoleEnum.ASSISTANT,
    );
    await expect(engine.resendMonotonicMessage(firstStoppedAssistant, "second edit")).resolves.toBe(
      true,
    );
    ...
    const secondRuntime = selectSessionTurnRuntime(
      turnRuntimeRegistry.value,
      sessionId,
      secondReplaceScope,
    );
  ...
  it("resendMonotonicMessage keeps the second replacement turn running instead of inheriting stopped state", async () => {
    ...
    const { engine, activeSession, activeTurnRuntime, sending, canStop, turnRuntimeRegistry } =
      createHarness({
        sessionId: "local-resend-second-stopped",
        stream,
        deps: { replaceSessionTurnApi, applySessionDetail },
    ...
    await expect(
      engine.resendMonotonicMessage(activeSession.value.messages[1], "second"),
    ).resolves.toBe(true);
    ...
    await expect(engine.resendMonotonicMessage(firstReplacementAssistant, "third")).resolves.toBe(
      true,
    );
    ...
    expect(secondPlaceholder).toEqual(
      expect.objectContaining({
        role: RoleEnum.ASSISTANT,
        content: "",
        pending: false,
  ...
  it("ignores stale message-level stopped state when explicit runtime state allows a second replacement", async () => {
    ...
    const stream = vi.fn(async (payload, onEvent) => {
      ...
      if (streamCallCount === 2) {
        emitChannelState(
          onEvent,
          "local-resend-stale-stop-replay",
          "dp-old-stopped",
          "user_stopped",
          {
            turnScopeId: payload.identity.turnScopeId,
    ...
    const firstAssistant = activeSession.value.messages.find(
      (message) =>
    ...
    const freshPlaceholder = [...activeSession.value.messages]
      ...
      .find(
        (message) =>
          ...
          message.turnScopeId === activeTurnRuntime.value.turnScopeId,
      );
    expect(freshPlaceholder).toEqual(
      expect.objectContaining({
        role: RoleEnum.ASSISTANT,
        pending: false,
  ...
  it("resendMonotonicMessage ignores stale stopped assistants after the new replacement user", async () => {
    ...
    await expect(engine.resendMonotonicMessage(staleStoppedAssistant, "new attempt")).resolves.toBe(
      true,
    );

    const replacementUser = activeSession.value.messages.find(
