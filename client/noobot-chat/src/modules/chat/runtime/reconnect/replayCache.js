 ...
 */
import { createTurnKey, parseTurnKey, resolveTurnIdentity } from "../engine/turnIdentity.js";
  ...
  return replayGroups
    .map(([replayKey, replayMessages]) => {
      ...
      const dialogProcessIds = new Set(
        identities.map((identity) => identity.dialogProcessId).filter(Boolean),
      );
      return {
        replayKey,
        dialogProcessId: dialogProcessIds.size === 1 ? [...dialogProcessIds][0] : "",
