...
### 1.3.6 Agent 运行中“自身产生”的消息（对话循环内追加）

这部分是你提到的关键点：除了 context 构建阶段的历史消息，`agent` 在一次对话循环中还会继续往 `messages` 追加新消息。

| 产生时机 | 代码位置 | 追加到 `messages` 的内容 | 是否影响同一轮后续调用 | 是否进入会话持久化（下轮可见） | 说明 |
| --- | --- | --- | --- | --- | --- |
| `no_tools` 模式模型返回后 | `turn/turn-executor.js` `messages.push(modelResponse)` | assistant 消息（AIMessage 结果对象） | 是 | 是（通过 `stateCommitter.pushAssistantMessage` 写入 turn store） | 该轮结束后会作为后续上下文的一部分 |
| `with_tools` 模式模型返回后 | `turn/turn-executor.js` `messages.push(ai)` | assistant 消息（含可能的 tool_calls） | 是 | 是（通过 `stateCommitter.pushAssistantMessage` 写入 turn store） | 工具链路入口消息 |
| 工具执行完成后 | `execution/state-committer.js` `messages.push(new ToolMessage(...))` | tool 结果消息 | 是 | 是（`pushToolResult` 同时写入 turn store） | 后续轮次会参与 tool-pair 合法性过滤 |
| reasoning-only 重试触发时 | `turn/turn-executor.js` `messages.push({ role:\"system\", content: buildReasoningRetrySystemMessage(...) })` | 临时 system 提示消息 | 是 | 不保证（主要用于当轮重试） | 用于“模型只返回思考无最终答案”时追加重试提示 |
| `before_llm_call` hook 注入 | hook 实现对 `ctx.messages` 的 `unshift/push/splice` | user/system/assistant 等注入消息 | 是 | 取决于注入实现是否同时落 turn store | 这就是“当前用户元信息后面还能有消息”的来源 |

运行中的真实顺序（简化）：

```text
初始 base messages（来自 buildContextMessages）
-> before_llm_call hooks 改写
-> invoke(filterForModelContext(messages))
-> messages.push(ai/assistant)
-> 若有工具调用：messages.push(tool result)
-> 下一次 llm 调用继续基于已增长的 messages
```
...
| 消息类别 | 是否过滤 | 是否裁剪 | 说明 |
| --- | --- | --- | --- |
| 历史基底消息（进入本轮前） | 是 | 是 | 会经过“第一次 session 历史筛选/裁剪 + 第二次 main_model recent 筛选/裁剪” |
| Agent 运行中新增消息（本轮 append） | 是 | 否 | 包括 `messages.push(ai/modelResponse)`、`messages.push(tool result)`、`before_llm_call` 注入；调用前只过 `filterForModelContext`，不再过 recent window |

一句话版本：

```text
agent 侧不是“所有消息都只过滤不裁剪”，而是：
历史基底 = 先裁剪再过滤；
运行中新增 = 只过滤不再裁剪。
```
