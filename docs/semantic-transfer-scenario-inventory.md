...
这些工具的业务结果本身需要作为可下载附件保留，因此直接调用 transfer adapter 的文本物化 API，并将 V2 envelope 放入结构化工具结果：

| 工具 | adapter | 策略 |
| --- | --- | --- |
| `web_to_data` | `materializeTextForToolResult` | `alwaysPersist=true`，生成 Markdown 附件；阈值内可同时返回 inline text |
| `media_to_data` | `materializeTextForToolResult` | 保存转写/分析文本附件 |
| `doc_to_data` | `materializeTextForToolResult` | 保存解析文本；已有解析产物用 `createExistingAttachmentTransferEnvelope` 复用 canonical identity |

这些工具不允许根据路径重新构造 attachment identity。复用已有附件时必须提供完整的 `attachmentId + sessionId + attachmentSource`。
...
detached 子 Session 返回时，从 canonical `turnMessages[*].transferEnvelopes` 聚合并按 `transferId` 去重；`process_content_task` 和 `process_connector_tool` 只传播这些既有 envelopes，不重新持久化或重建附件。
