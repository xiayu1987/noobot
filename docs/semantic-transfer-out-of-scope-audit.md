...
- 文件：`agent/src/system-core/bot-manage/session/session-execution-engine.js`
- 位置：`_createGeneratedArtifactPersister()`
- 当前调用：`persistTransferArtifacts(...)`
- 当前用途：通用 generated artifact persister，把生成物保存为附件，再取 attachment metas。
- 判断：**偏离原始需求**。
- 原因：这不是工具输入/输出超限转换、不是 workflow 子 agent 信息流转、也不是 harness 消息管理；更像通用附件持久化。
- 建议：回退为直接调用 `attachmentService.ingestGeneratedArtifacts(...)`，不要通过 semantic-transfer。
...
- 判断：**偏离原始需求**。
- 原因：这是模型输出媒体附件持久化，不属于三类 semantic-transfer 信息转换场景。
- 建议：改为普通附件保存路径；只有当模型输出需要作为“工具信息传递/阶段明细/子 agent 结果流转”转换时，才使用 semantic-transfer。
...
- 判断：**边界偏宽 / 建议收敛**。
- 说明：
  - 如果按“工具输出内容转换”宽口径，可勉强纳入工具输出。
  - 但它不是“工具原始返回超限后保存附件”，更像生成图片附件持久化。
- 建议：如果只是保存生成图片，走普通 `attachmentService`；如需返回给模型的文本结果超限/需转换，再由 `transferToolMessage(...)` 或工具输出 compact 处理。
...
- 建议：改为普通附件保存路径；connector 工具的文本返回超限时再走工具输出 semantic-transfer。
...
- 建议：
  - 如果这是“子 agent 最终结果流转”，应优先收口到 `transferSubAgentMessages(...)` 这类语义入口。
  - 如果只是普通异步任务结果附件保存，应回退到 `attachmentService.ingestGeneratedArtifacts(...)`。
...
- 当前调用：`getTransferAttachmentMetas(...)`
- 当前用途：从消息里的 transfer envelopes 中提取附件，并提升/合并到最终 assistant 消息。
- 判断：**消费侧边界偏宽**。
- 原因：该逻辑面向所有 transfer envelope，而不是仅限工具 overflow、workflow 子 agent 或 harness 阶段/最终消息。
- 建议：
  - finalizer 不应把 semantic-transfer 当成通用附件聚合协议。
  - 若确实需要提升，只应对明确的 generationSource/reason 白名单处理，例如 `tool_result_overflow`、`workflow_node_agent_result`、`harness_*` 等授权场景。

---

## 2. 根因

当前 `runtime.sharedTools.semanticTransfer` 暴露了过宽的通用能力：
...
这些 API 让业务代码容易把 semantic-transfer 当成：
...
- [ ] `session-execution-engine.js`：通用 generated artifacts 保存从 `persistTransferArtifacts` 回退到 `attachmentService.ingestGeneratedArtifacts(...)`。
- [ ] `artifact-service.js`：LLM 输出媒体附件保存从 semantic-transfer 收回。
- [ ] `multimodal-generate-tool.js`：图片生成附件保存评估是否回退到普通附件服务。
- [ ] `tool-access-connector.js`：email connector 附件保存从 semantic-transfer 收回。
- [ ] `collab-artifact-persist.js`：区分“子 agent 结果流转”和“普通异步结果附件保存”，分别走 `transferSubAgentMessages(...)` 或 attachmentService。
- [ ] `finalizer.js`：transfer 消费改为授权场景白名单，不做全局 transfer 附件提升。
- [ ] `runtime-environment-builder.js`：收窄 `sharedTools.semanticTransfer` 暴露面，不再把 persist/materialize 类方法作为普通业务 API。
