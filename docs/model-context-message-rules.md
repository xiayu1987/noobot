...
## 2. harness 插件非主流程模型请求上下文规则

harness 插件给非主流程模型请求的上下文第 1 段统一为：

```text
1. 和主流程最终给模型的消息一致的 agent 上下文（systemMessages + historyMessages + incrementalMessages）
2. capability 消息计划声明的协议、策略和流程上下文
```

注意：

- 不做非主流程侧裁剪。
- 文本协议和稳定 workflow policy 使用 `system`；当前请求和交互输入使用 `user`。
- 报告、清单、计划、上一阶段结果等流程上下文的 role 和顺序由 capability 的消息计划声明，不在 agent 或调用点按内容名称重新推断。
- 验收类流程可由其消息计划声明不携带 agent 上下文；协议及流程上下文仍由同一消息计划负责排序和定角。
- 消息计划是各 capability 请求结构的唯一事实源；调用点只渲染计划，不复制角色分配规则。
...
phase acceptance separate model 的具体顺序以其 capability 消息计划为准，结构为：

```text
1. agent 上下文（systemMessages + historyMessages + incrementalMessages）
2. capability 消息计划（协议、稳定策略、流程上下文、请求与约束）
```
