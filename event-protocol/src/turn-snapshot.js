...
export function createTurnSnapshotEnvelope(snapshot, {
  eventId,
  producer = { type: "domain-service", id: "turn-snapshot-query" },
} = {}) {
...
export function validateTurnSnapshotEnvelope(envelope = {}) {
  ...
  if (envelope?.identity?.eventType !== TURN_SNAPSHOT_WIRE_EVENT) errors.push("invalid_snapshot_wire_event");
  if (envelope?.ordering?.domain !== TURN_SNAPSHOT_SEQUENCE_DOMAIN) errors.push("sequence_domain_mismatch");
  if (envelope?.ordering?.scopeId !== envelope?.identity?.sessionId) errors.push("sequence_scope_mismatch");
  if (envelope?.payload?.sessionId !== envelope?.identity?.sessionId) errors.push("snapshot_session_identity_mismatch");
  if (Number(envelope?.payload?.sequence) !== Number(envelope?.ordering?.sequence)) errors.push("snapshot_sequence_mismatch");
  if (envelope?.payload?.commandId !== envelope?.causality?.commandId) errors.push("snapshot_command_identity_mismatch");
