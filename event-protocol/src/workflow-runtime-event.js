...
export function validateWorkflowRuntimeEnvelope(envelope = {}) {
  ...
  if (eventType === WORKFLOW_RUNTIME_EVENT.NODE_STATE) {
    ...
    if (Object.hasOwn(payload, "sessionId") || Object.hasOwn(payload, "parentSessionId")) errors.push("invalid_node_session_field");
    if (!text(payload?.status)) errors.push("missing_node_status");
    if (!Number.isInteger(Number(envelope?.ordering?.revision)) || Number(envelope.ordering.revision) < 1)
  ...
  if (eventType === WORKFLOW_RUNTIME_EVENT.SESSION_SNAPSHOT) {
    ...
    if (Object.hasOwn(payload, "sessionId") || Object.hasOwn(payload, "parentSessionId")) errors.push("invalid_snapshot_session_field");
    if (!Number.isInteger(Number(envelope?.ordering?.aggregateVersion)) || Number(envelope.ordering.aggregateVersion) < 1)
      errors.push("invalid_aggregate_version");
    if ((Array.isArray(payload?.messages) ? payload.messages : []).some((message) => !text(message?.messageId)))
...
export function compareWorkflowRuntimeFacts(incoming = {}, current = {}, { defaultDomain = "" } = {}) {
  ...
  if (incomingRevision !== currentRevision) {
    return Object.freeze({ comparable: true, order: incomingRevision > currentRevision ? 1 : -1, incomingDomain, currentDomain });
  }
