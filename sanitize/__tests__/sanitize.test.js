...
test("secret assignments stop at sentence punctuation", () => {
  const input = "token=88b8c151167f53b346ae705c37dc3ad0，SHA-256=ebaddf93，完成。";
  const output = sanitizeSecrets(input);
  assert.equal(output, "token=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx，SHA-256=ebaddf93，完成。");
  assert.equal(
    sanitizeSecrets('token="highEntropy,Value_1234567890"'),
    'token="xxxxxxxxxxx,xxxxx_xxxxxxxxxx"',
  );
});
