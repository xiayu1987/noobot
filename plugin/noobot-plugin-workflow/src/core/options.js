...
function normalizeHookPoint(input = "") {
  const hookPoint = String(input || "").trim();
  if (hookPoint === WORKFLOW_BOT_HOOK_POINTS.AFTER_AGENT_DISPATCH) {
    return hookPoint;
  }
  return WORKFLOW_BOT_HOOK_POINTS.AFTER_AGENT_DISPATCH;
}
...
export function normalizeOptions(input = {}) {
  ...
  return {
    ...
    hookPoint: normalizeHookPoint(source?.hookPoint),
    semanticPrompt:
