...
const WORKFLOW_I18N_TEXT = Object.freeze({
  [WORKFLOW_LOCALE.ZH_CN]: Object.freeze({
    ...
    workflowInputAttachmentsPlanHint2:
      "ATTACHMENT 格式：ATTACHMENT id=\"局部别名\" attachmentId=\"事实ID\" sessionId=\"会话ID\" attachmentSource=\"来源\" name=\"附件名\" path=\"可读路径\" mimeType=\"MIME\"。",
  ...
  [WORKFLOW_LOCALE.EN_US]: Object.freeze({
    ...
    workflowInputAttachmentsPlanHint2:
      "ATTACHMENT format: ATTACHMENT id=\"local alias\" attachmentId=\"canonical id\" sessionId=\"session id\" attachmentSource=\"source\" name=\"Attachment Name\" path=\"Readable Path\" mimeType=\"MIME\".",
    workflowInputAttachmentsPlanHint3:
...
const DEFAULT_SEMANTIC_PROMPT_BY_LOCALE = Object.freeze({
  [WORKFLOW_LOCALE.ZH_CN]: [
    ...
    "",
    "DSL 指令：",
    "- ATTACHMENT id=\"局部别名\" attachmentId=\"...\" sessionId=\"...\" attachmentSource=\"...\" name=\"...\" path=\"...\" [mimeType=\"...\"]",
    ...
    "- ATTACHMENT 的 id 仅是当前 DSL 内局部别名；attachmentId、sessionId、attachmentSource 必须逐字使用用户附件列表里的事实身份，path 使用可读路径。",
    ...
    "示例：",
    "WORKFLOW_DSL/1",
    "ATTACHMENT id=\"file_001\" attachmentId=\"att_001\" sessionId=\"session_001\" attachmentSource=\"user\" name=\"用户附件.pdf\" path=\"/workspace/attachments/用户附件.pdf\" mimeType=\"application/pdf\"",
    "NODE id=start type=state stateType=start name=\"开始\"",
    "NODE id=branch type=state stateType=branch name=\"并发分叉\"",
    "NODE id=a type=action name=\"任务A\" task=\"读取用户附件并完成任务A\" attachments=\"file_001\"",
  ...
  [WORKFLOW_LOCALE.EN_US]: [
    ...
    "",
    "DSL directives:",
    "- ATTACHMENT id=\"local alias\" attachmentId=\"...\" sessionId=\"...\" attachmentSource=\"...\" name=\"...\" path=\"...\" [mimeType=\"...\"]",
    "- NODE id=... type=state|action name=\"...\" [stateType=start|end|branch|merge] [task=\"...\"] [attachments=\"...\"]",
    "- EDGE from=... to=...",
    "- AUTO type=submit stepIndex=0",
    "- END",
    ...
    "- ATTACHMENT id is only a local DSL alias; attachmentId, sessionId, and attachmentSource must exactly copy the canonical identity from the user attachment list; path is display-only.",
    ...
    "Example:",
    "WORKFLOW_DSL/1",
    "ATTACHMENT id=\"file_001\" attachmentId=\"att_001\" sessionId=\"session_001\" attachmentSource=\"user\" name=\"user-attachment.pdf\" path=\"/workspace/attachments/user-attachment.pdf\" mimeType=\"application/pdf\"",
    "NODE id=start type=state stateType=start name=\"Start\"",
    "NODE id=branch type=state stateType=branch name=\"Branch\"",
    "NODE id=a type=action name=\"Task A\" task=\"Read user attachment and finish task A\" attachments=\"file_001\"",
