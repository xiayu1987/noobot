...
export function createRunningAssistantPlaceholderViewModel(
  ...
  const matchesTurn = (item = {}) => {
    ...
    if (normalizedDialogProcessId && itemDialogId)
      return itemDialogId === normalizedDialogProcessId;
    return !scopeId && !normalizedDialogProcessId;
  };
  const matchingUser = source.find(
    (item = {}) => text(item?.role).toLowerCase() === "user" && matchesTurn(item),
  );
  if (!matchingUser) return null;
  if (
    source.some((item = {}) => text(item?.role).toLowerCase() === "assistant" && matchesTurn(item))
  )
    return null;
  return {
    ...
    dialogProcessId: text(
      matchingUser?.dialogProcessId ||
        matchingUser?.metadata?.dialogProcessId ||
...
export function buildUnifiedSessionDetail({
  ...
  if (childExecutionIds.length && typeof selectExecutionDetail === "function") {
    ...
    if (executionDetail) {
      ...
      const isolatedSessionDoc =
        isolatedNodeSessionId && typeof selectSessionMessages === "function"
      ...
      const executionOwnsIsolatedSession = Boolean(
        ...
        !executionSessionId ||
        executionSessionId === isolatedNodeSessionId,
      );
      ...
      const rawMessages = hasIsolatedSessionProjection
        ? Array.isArray(isolatedSessionDoc.messages)
          ? isolatedSessionDoc.messages
          : []
    ...
    ? messages.filter((messageItem = {}) => {
        const messageTurnScopeId = text(
          messageItem?.turnScopeId ||
            messageItem?.metadata?.turnScopeId ||
            messageItem?.pluginMeta?.turnScopeId,
        );
  ...
  const runningPlaceholderViewModel = createRunningAssistantPlaceholderViewModel(
    scopedRawMessages,
    {
      sessionId,
      turnScopeId,
