...
const I18N_RUNTIME_LABELS = Object.freeze({
  [LOCALE.ZH_CN]: Object.freeze({
    ...
    guidanceAnalysisPromptGoal:
      "请判断当前最该做什么、发现了什么问题、应如何解决。",
    guidanceAnalysisOutputConstraint:
      "只用一句话概括，覆盖下一步、问题和解决方式；没有明确问题也要给出下一步。",
    dynamicPolicyPromptProtocolInstruction:
  ...
  [LOCALE.EN_US]: Object.freeze({
    ...
    guidanceAnalysisPromptGoal:
      "Decide what should be done now, what problem is visible, and how to resolve it.",
    guidanceAnalysisOutputConstraint:
      "Use exactly one concise sentence covering the next step, the problem, and the fix; if no clear problem is visible, still give the next step.",
    dynamicPolicyPromptProtocolInstruction:
