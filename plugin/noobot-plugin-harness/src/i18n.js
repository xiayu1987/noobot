...
const I18N_RUNTIME_LABELS = Object.freeze({
  [LOCALE.ZH_CN]: Object.freeze({
    ...
    harnessPolicyGeneralExecutionFirstPrompt:
      "Noobot Harness 通用策略：遵守用户隔离；先读必要上下文，做最小切片可逆动作；循环执行 -> 验证/反馈 -> 修正 -> 继续，不断推进任务。验证是完成条件：优先跑相关测试/检查/构建，失败先修复重试；无法验证必须说明原因。仅在不可逆/破坏性、安全隐私、生产/资金、高成本外部动作或需求冲突时停下确认。最终回复简洁说明结果与验证。",
    ...
    harnessPolicyTextExecutionFirstPrompt:
      "Noobot Harness 文本场景策略：遵守用户隔离；文本模式核心是多产出，建议外部文本拿到就保真消费，优先提取关键文本内容、来源文件路径、事实、约束、关键依据、交付要求和可复用片段；按可交付文本批次持续产出；内容复杂时可以边查/边搜/边核对，边写/边产出，不必等全部资料完全收集后才开始形成产物；轻量检查覆盖、来源和格式后继续扩展，不因普通不确定性长期等待；最终回复简洁说明文本处理结果、路径、产出和检查。",
    ...
    guidanceSummaryTextOutputNextActionRules:
      "文本场景策略的 [NEXT_EXECUTION_SUGGESTION] 必须且只允许输出 1 个 [NEXT_ACTION] 文本块；核心策略：文本模式核心是多产出，建议外部文本拿到就保真消费，优先提取关键文本内容、来源文件路径、事实、约束、关键依据、交付要求和可复用片段；按可交付文本批次持续产出；内容复杂时可以边查/边搜/边核对，边写/边产出，不必等全部资料完全收集后才开始形成产物；action 只能是 consume、extract、draft、expand、revise、verify、ask_user、final 之一；target 必须是具体文本来源、产物、处理对象或需要询问用户的问题；用 batch_mode=deliverable_text_batch 表明按可交付文本批次持续推进；batch_scope 写本轮批次范围；output_goal 写本轮要形成的文本产物；coverage_check 写来源覆盖、关键事实或格式检查结果，没有则用 -；result_state 写 done、needs_more_text、needs_fix、blocked 或 unknown；artifact_path 有产物路径则填写，否则用 -；reason 必须简短；blocking=true 仅用于合规、安全、承诺、不可逆、高成本外部动作或需求冲突，否则用 blocking=false 并继续扩展产出。",
  ...
  [LOCALE.EN_US]: Object.freeze({
    ...
    harnessPolicyGeneralExecutionFirstPrompt:
      "Noobot Harness general policy: enforce user isolation; read necessary context, then take the smallest reversible action; loop execute -> verify/observe -> fix -> continue. Verification is required for completion: prefer relevant tests/checks/builds; fix failures and retry, or state why verification was impossible. Stop for confirmation only for irreversible/destructive, security/privacy, production/money, costly external actions, or requirement conflicts. Final response: concise result and validation.",
    harnessPolicyGeneralRiskFirstPrompt:
