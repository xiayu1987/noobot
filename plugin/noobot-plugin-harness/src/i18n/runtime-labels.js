...
export const I18N_RUNTIME_LABELS = Object.freeze({
  [LOCALE.ZH_CN]: Object.freeze({
    ...
    responsibilityConstraintTemplate:
      "只返回「{stage}」结果；有文本协议时必须遵守。注：此请求是协助请求，无需执行工具。",
    ...
    guidanceAnalysisPromptGoal:
      "请根据当前执行结果，用一句自然、专业的话概括当前进展，让人一眼就知道现在做到哪了、有没有新的问题情况疑点风险、接下来该做什么，并具有明确的指引性。不要自己执行任务。避免固定模板、分点或机械套话。如无变化，请回复“分析中”。",
  ...
  [LOCALE.EN_US]: Object.freeze({
    ...
    responsibilityConstraintTemplate:
      "Return only the {stage} result using the text protocol when provided. Note: this is an assistance request; no tool execution is needed.",
    responsibilityScenarioMismatchProtocol:
