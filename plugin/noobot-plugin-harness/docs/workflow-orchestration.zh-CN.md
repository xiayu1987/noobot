...
事件自身记录发送域和 Hook 点；Planning、Guidance、Acceptance、Review 都使用该事件。
...
- `candidateActions`: 本轮扫描到的可候选动作集合
- `deferredActions`: 本轮未执行、延后处理的动作集合
- `blockedActions`: 当前被显式 blocker 阻塞的动作集合
- `blockedReasons`: blocker 原因码集合
- `triggeredActions`: 显式触发动作的投影；没有独立列表时取 `candidateActions`
- `pending`: 状态快照
  - `summary`
  - `guidance`
  - `analysis`
...
`candidateActions`、`deferredActions` 和 `blockedReasons` 是调度事实；`triggeredActions` 只是供消费者读取触发子集的事件投影，不定义第二套调度决策。
...
- Plan Update 触发：
  - `state.counters.planUpdateTurns` 达到 `WORKFLOW_PARAMS.modeThresholds.<full|programming|text>.planning.planUpdate.triggerTurnsThreshold`
- Phase Acceptance 调度阈值：
  - `state.counters.phaseAcceptanceTurns` 达到 `WORKFLOW_PARAMS.modeThresholds.<full|programming|text>.acceptance.phase.triggerTurnsThreshold`
...
- Hook 点：
  ...
  - `tool_call_error`
- Summary 触发：
  - 轮次触发：`state.counters.summaryTurns` 超过当前模式的 `guidance.summary.turnsThreshold`
  - 字符触发：未小结字符数超过 `WORKFLOW_PARAMS.guidance.summary.messageCharsThreshold`
  - 工具爆发触发：启用后在 `after_tool_calls` 根据工具调用数量触发
- 小结后仍溢出的处理策略（`SUMMARY_POLICY.OVERFLOW_POLICY`）：
  - `FORCE_ACCEPTANCE_WHEN_STILL_OVERFLOW`
- Analysis 触发：
  - `state.counters.analysisTurns` 达到当前模式的 `guidance.analysis.turnsThreshold`
- 失败阈值来自 `WORKFLOW_PARAMS.guidance.failureThreshold`。
...
各 domain 在 `workflow_priority_decision.pending` 中使用统一结构，并可省略不归本域所有的字段：
...
`runWorkflowLifecycle(...)` 保证即使执行抛错也写入 `workflow_execution_result`：
...
Review controller 从 `WORKFLOW_PARAMS.review.hooks` 读取以下白名单：
...
| Shared facade          | 稳定的 shared 聚合导出                    | `src/capabilities/handlers/shared.js`                                      |
