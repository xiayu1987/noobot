...
- Plan Update 触发：
  - `state.counters.planUpdateTurns >= PLAN_UPDATE_TRIGGER_TURNS_THRESHOLD`（按模式读取 `WORKFLOW_PARAMS.modeThresholds.<full|programming>.planning.planUpdate`）
- Phase Acceptance 调度阈值：
  - `state.counters.phaseAcceptanceTurns >= PHASE_ACCEPTANCE_TRIGGER_TURNS_THRESHOLD`（按模式读取 `WORKFLOW_PARAMS.modeThresholds.<full|programming>.acceptance.phase`）
