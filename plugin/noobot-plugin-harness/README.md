...
| `summaryDetailSaveToAttachment` / `saveSummaryDetailToAttachment` | `false` | Controls Harness summary detail delivery. `false` injects the full summary, including detail, back to the main agent; `true` keeps the previous behavior and saves detail as generated attachment(s), injecting only overview plus detail paths. |
