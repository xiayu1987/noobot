...
{
  "toolPolicy": {
    "denyToolNames": ["execute_script", "execute_native_script"]
  }
}
```
...
| `capabilityModelTimeoutMs`                                        | `180000`                             | Deadline for each read-only separate-model capability call. Timeout aborts that capability call and is handled as a capability failure; it does not consume the main hook deadline.                                                                                                                                                          |
...
| `denyToolNames`                                                   | `["execute_script", "task_summary"]` | Optional plugin-level deny list. Engine appends it into `runConfig.toolPolicy.denyToolNames` via the unified policy field.                                                                                                                                                                                                                   |
