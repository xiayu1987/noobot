...
test("harness policy prompt is promoted to system when stale policy exists in messageBlocks", async () => {
  ...
  assert.equal(
    ctx.modelContext.messageBlocks.incremental.filter(
      (item = {}) =>
...
test("harness policy prompt matrix exposes scenario without workflow mode", async () => {
  ...
  const textPrompt = await buildInjectedPolicy({}, { runConfig: { scenario: "text" } });
  ...
  assert.doesNotMatch(
    defaultPrompt,
    new RegExp("执行" + "优先|" + "风险" + "优先|risk first", "i"),
  );
...
test("harness policy prompt is injected for normal first llm call", async () => {
  ...
  assert.equal(
    ctx.modelContext.messages.some((message = {}) =>
      String(message.content || "").includes("HARNESS_POLICY_SELECTION"),
    ),
    true,
  );
...
test("harness policy selection resolver maps scenario to i18n keys", () => {
  assert.deepEqual(resolvePolicyPromptSelection({}, {}).policyPromptId, "harness_policy/general");
  assert.equal(resolvePolicyPromptSelection({}, {}).i18nKey, "harnessPolicyGeneralPrompt");
  assert.equal(
