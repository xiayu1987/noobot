...
const manifest = {
  ...
  requires: { ports: ["hooks.register", "hooks.emit", "frontend.contribute"], permissions: [], authenticatedRoutes: [] },
...
test("manifest V2 is strict and surface-owned", () => {
  ...
  assert.equal(requireDeclaredPluginHook(parsed, "agent", "agent.before_turn"), "agent.before_turn");
  assert.equal(requireDeclaredPluginHookEmission(parsed, "agent", "workflow.node_agent_execute"), "workflow.node_agent_execute");
  assert.equal(requireDeclaredFrontendContribution(parsed, "example-card", "message.card.pre").id, "example-card");
