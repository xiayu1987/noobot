...
export async function discoverNoobotPluginManifests({ pluginRootDir = "" } = {}) {
  ...
  try {
    ...
    if (error?.code !== "ENOENT" && error?.code !== "ENOTDIR") {
      console.warn("[plugin-runtime] plugin manifest discovery failed", { root, error });
    }
    return [];
  }
  const manifests = [];
  for (const directoryEntry of directoryEntries) {
    ...
    try {
      ...
      if (error?.code !== "ENOENT" && error?.code !== "ENOTDIR") {
        console.warn("[plugin-runtime] plugin manifest access failed", { manifestPath, error });
      }
    }
  }
...
export async function loadNoobotPlugins(options = {}) {
  ...
  const skipped = [];
  const errors = [];
  const lifecycleEvents = discovered.map((item) =>
    Object.freeze({
  ...
  for (const item of discovered) {
    try {
      ...
      if (!manifestContributesToSurface(manifest, normalized.surface)) {
        skipped.push({
          pluginId: manifest.id,
          reason: `no_${normalized.surface}_contributions`,
          ...item,
        });
        continue;
      }
      ...
      if (typeof moduleNamespace.activate !== "function") {
        throw new Error(
          `plugin surface entry must export activate: ${manifest.id}/${normalized.surface}`,
        );
      }
      lifecycleEvents.push(
        Object.freeze({
...
export function resolveLoadedNoobotPlugin(runtime = null, pluginId = "") {
  ...
  return normalized && runtime?.registry instanceof Map
    ? runtime.registry.get(normalized) || null
    : null;
}
...
export function listPluginsContributingHook(runtime = null, point = "") {
  ...
  return listLoadedNoobotPluginEntries(runtime).filter((entry) =>
    (contributionsForSurface(entry.manifest, entry.surface)?.hooks?.registers || []).some(
      (item) => item.point === normalized,
    ),
  );
}
